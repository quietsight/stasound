...
<% '--Updater File-- %>
<% pageTitle = "CyberSource Gateway -  eCheck Database Update" %>
<% Section="paymntOpt" %>
...
IF request("action")="sql" then
	if request.querystring("hmode")="2" then
		...
		if SSIP="" or UID="" or PWD="" then
			response.redirect "upddbCyberSource.asp?mode=3"
			response.End
		end if
		...
		if instr(ucase(scDSN),"DSN=") then
			response.redirect "upddbCyberSource.asp?mode=1"
			response.End
		end if
		call openDb()
	end if
	...
	'//  Create column pcPay_Cys_eCheck for table pcPay_CyberSource
	query="ALTER TABLE [pcPay_CyberSource] ADD [pcPay_Cys_eCheck] [int] NULL"
	...
	if err.number <> 0 then
		if instr(ucase(Err.Description),"COLUMN NAMES IN EACH TABLE MUST BE UNIQUE") then
			...
			ErrStr=ErrStr&"Unable to update TABLE pcPay_CyberSource - Error: "&Err.Description&"<BR>"
			err.number=0
			iCnt=iCnt+1
		end if
	else
		query="UPDATE pcPay_CyberSource SET pcPay_Cys_eCheck=0;"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp.execute(query)
		query="ALTER TABLE [pcPay_CyberSource] ADD CONSTRAINT [DF_pcPay_CyberSource_pcPay_Cys_eCheck] DEFAULT (0) FOR [pcPay_Cys_eCheck];"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp.execute(query)
	end if

	'//  Create column pcPay_Cys_eCheckPending for table pcPay_CyberSource 
	query="ALTER TABLE [pcPay_CyberSource] ADD [pcPay_Cys_eCheckPending] [int] NULL"
	set rs=server.CreateObject("ADODB.RecordSet")
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		if instr(ucase(Err.Description),"COLUMN NAMES IN EACH TABLE MUST BE UNIQUE") then
		...
		query="UPDATE pcPay_CyberSource SET pcPay_Cys_eCheckPending=0;"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp.execute(query)
		query="ALTER TABLE [pcPay_CyberSource] ADD CONSTRAINT [DF_pcPay_CyberSource_pcPay_Cys_eCheckPending] DEFAULT (0) FOR [pcPay_Cys_eCheckPending];"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp.execute(query)
	end if
...
ELSE
	if request("action")="access" then
		...
		query="ALTER TABLE pcPay_CyberSource ADD pcPay_Cys_eCheck LONG NULL;"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=connTemp1.execute(query)
		if err.number <> 0 then
			call TrapError("pcPay_CyberSource")
		else
			query="UPDATE pcPay_CyberSource SET pcPay_Cys_eCheck=0;"
			set rs=server.CreateObject("ADODB.RecordSet")
			set rs=connTemp1.execute(query)
			query="ALTER TABLE [pcPay_CyberSource] ALTER COLUMN [pcPay_Cys_eCheck] LONG NULL DEFAULT '0';"
			set rs=server.CreateObject("ADODB.RecordSet")
			set rs=connTemp1.execute(query)
		end if
		
		query="ALTER TABLE pcPay_CyberSource ADD pcPay_Cys_eCheckPending LONG NULL;"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=connTemp1.execute(query)
		if err.number <> 0 then
			call TrapError("pcPay_CyberSource")
			...
			set rs=server.CreateObject("ADODB.RecordSet")
			set rs=connTemp1.execute(query)
			query="ALTER TABLE [pcPay_CyberSource] ALTER COLUMN [pcPay_Cys_eCheckPending] LONG NULL DEFAULT '0';"
			set rs=server.CreateObject("ADODB.RecordSet")
			set rs=connTemp1.execute(query)
		end if
...
<form action="upddbCyberSource.asp" method="get" name="form1" class="pcForms">
<% 
'mode=Edit&id=219&gwchoice=32
if mode="complete" then
	response.redirect "AddModRTPayment.asp?mode="&tmp_mode&"&id="&tmp_id&"&gwchoice=32"
	response.end
else %>
	...
	<table class="pcCPcontent" style="width:600px;" align="center">
		...
		<tr>
			<td>
				...
				<span style="font-weight: bold">You must update your database before you can add/modify CyberSource Gateway to support eCheck.<u></u></span><br>
				In order to activate CyberSource real-time payment gateway you will need to update your ProductCart database to add the required table.</div>				<p><strong><br>
				    <br>
			...
			<% if scDB="Access" then %>
					<tr>
						<td align="center">
						  <input type="button" name="access" value="Update Your ProductCart MS Access Database" onClick="location='upddbCyberSource.asp?action=access&mode=<%=tmp_mode%>&id=<%=tmp_id%>';" class="submit2">
						</td>
					</tr>
			<% 
				else
			...
			<% end if %>
				<tr>
					...
					<% if request.querystring("mode")="1" OR request.querystring("mode")="3" then %>
						...
						<% else %>
					  <input type="button" name="access2" value="Update Your ProductCart MS SQL Database" onClick="location='upddbCyberSource.asp?action=sql&mode=<%=tmp_mode%>&id=<%=tmp_id%>';" class="submit2">
						<% end if %>
					</td>
			</tr>
			<% end if %>
