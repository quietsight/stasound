	...
	While Not rs.EOF %>
		<table border="0" cellspacing="0" cellpadding="0" width="100%" align="center">
			<tr> 
				...
				<td width="43%" height="71" valign="bottom" align="right"> 
			...
			<tr> 
				...
				<td rowspan="2" width="50%" valign="top">
					<table align="right" width="95%" cellpadding="5" cellspacing="0" class="invoice">
						...
						<%if pcOrdKey<>"" then%>
						<tr> 
							<td class="invoice">ORDER CODE: <b><%=pcOrdKey%></b></td>
						</tr>
						<%end if%>
  ...
  '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	set rstemp=nothing%>
					...
					If RewardsActive <> 0 Then
						...
						<tr>
						<td class="invoice" align="right" colspan="3"><%=RewardsLabel%></td>
...
%>
	...
	<form action="OrdInvoice.asp" method="post" name="invoice" class="pcForms" onSubmit="if (this.oformat[1].checked) this.action='OrdInvoicePDF.asp'; return(true);">
		<table class="pcCPcontent">
			...
			<tr>
				...
				<tr>
					<td>If no invoice number is specified, the order number auto-generated by ProductCart will be used as the invoice number.</td>
				</tr>
				<tr>
					<td>
					#
				...
				<%
						...
						call opendb()
								...
								query="SELECT idOrder from creditCards WHERE idOrder=" & qry_ID & ";"
								Set rs=Server.CreateObject("ADODB.Recordset")
								Set rs=connTemp.execute(query)
								if NOT rs.eof then
									myDisplay=1
								end if
								query="SELECT idauthorder from authorders WHERE idOrder=" & qry_ID & ";"
								Set rs=Server.CreateObject("ADODB.Recordset")
								Set rs=connTemp.execute(query)
								if NOT rs.eof then
									myDisplay=1
								end if
								query="SELECT idauthorder from pcPay_EIG_Authorize WHERE idOrder=" & qry_ID & ";"
								Set rs=Server.CreateObject("ADODB.Recordset")
								Set rs=connTemp.execute(query)
								if NOT rs.eof then
									myDisplay=1
								end if
								query="SELECT idpfporder from pfporders WHERE idOrder=" & qry_ID & ";"
								Set rs=Server.CreateObject("ADODB.Recordset")
								Set rs=connTemp.execute(query)
								if NOT rs.eof then
									myDisplay=1
								end if
								query="SELECT idOrder from netbillorders WHERE idOrder=" & qry_ID & ";"
								Set rs=Server.CreateObject("ADODB.Recordset")
								Set rs=connTemp.execute(query)
								if NOT rs.eof then
									myDisplay=1
								end if
								query="SELECT idOrder from pcPay_USAePay_Orders WHERE idOrder=" & qry_ID & ";"
								Set rs=Server.CreateObject("ADODB.Recordset")
								Set rs=connTemp.execute(query)
								if NOT rs.eof then
									myDisplay=1
								end if
								query="SELECT * from customCardOrders WHERE idOrder=" & qry_ID & ";"
								Set rs=Server.CreateObject("ADODB.Recordset")
								Set rs=connTemp.execute(query)
								if NOT rs.eof then
									myDisplay=1
						...
						if myDisplay=1 then %>
								...
								<tr>
									<td><input type="radio" name="showCCInfo" value="NO" checked class="clearBorder"> No, I do not want card information printed on the invoice.</td>
								</tr>
								<tr>
									<td><input type="radio" name="showCCInfo" value="YES" class="clearBorder"> Yes, I want card information printed on the invoice.</td>
								</tr>
								<tr>
									<td class="pcCPspacer"></td>
								</tr>
						...
						<% else %>
							<input type="hidden" name="showCCInfo" value="NO">
						<% end if %>
			...
			<tr>
				<td><input type="radio" name="showAdminComments" value="NO" checked class="clearBorder"> No, don't print the administrator's comments on the invoice.</td>
			</tr>
			<tr>
				<td><input type="radio" name="showAdminComments" value="YES" class="clearBorder"> Yes, print the administrator's comments on the invoice.</td>
			</tr>
            <tr>
                <td class="pcCPspacer"></td>
            </tr>
			<tr>
				<td><b>Choose Format</b></td>
			</tr>
			<tr>
				<td><input type="radio" name="oformat" value="0" checked class="clearBorder"> HTML</td>
			</tr>
			<tr>
				<td><input type="radio" name="oformat" value="1" class="clearBorder"> PDF (Beta)</td>
			<tr>
				<td class="pcCPspacer"><hr></td>
			</tr>
				<tr>
					<td align="center"><input type="submit" name="GO" value="Generate" class="submit2"></td>
				</tr>
			</table>
	</form>
	</div>
