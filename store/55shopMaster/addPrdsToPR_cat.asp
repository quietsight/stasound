...
if request("action")="add" then
	if (request("prdlist")<>"") and (request("prdlist")<>",") then
		...
		For i=lbound(prdlist) to ubound(prdlist)
			...
			If (id<>"0") and (id<>"") then
				if pidcode<>"0" then
					query="INSERT INTO pcCPFProducts (pcCatPro_ID,idproduct) values (" & pidcode & "," & ID & ")"
	...
	if pidcode=0 then
		...
		response.redirect "ModPromotionCat.asp?idcategory=" & pIDCategory
	end if
end if
%>
	...
	<table id="FindProducts" class="pcCPcontent">
		<tr>
			...
			<%
				...
				src_DisplayType=1
				src_ShowLinks=0
				src_FromPage="addPrdsToPR_cat.asp?idcode=" & pidcode & "&idcategory=" & pIDCategory
				src_ToPage="addPrdsToPR_cat.asp?action=add&idcode=" & pidcode & "&idcategory=" & pIDCategory
				src_Button1=" Search "
				...
				UseSpecial=1
				session("srcprd_from")=""
				session("srcprd_where")=" AND (products.idProduct NOT IN (select idproduct FROM pcCPFProducts WHERE pcCatPro_id=" & pidcode & ")) "
			%>
