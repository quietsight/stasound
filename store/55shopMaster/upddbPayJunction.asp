...
<% '--Updater File-- %>
<% pageTitle = "PayJunction Gateway - Database Update" %>
<% Section = "" %>
<%PmAdmin=19%> 
...
IF request("action")="sql" then
	if request.querystring("hmode")="2" then
		...
		if SSIP="" or UID="" or PWD="" then
			response.redirect "upddbPayJunction.asp?mode=3"
			response.End
		end if
		...
		if instr(ucase(scDSN),"DSN=") then
			response.redirect "upddbPayJunction.asp?mode=1"
			response.End
		end if
		call openDb()
	end if
	...
	'**** Create table pcPay_PayJunction ***************************************
	query="CREATE TABLE [dbo].[pcPay_PayJunction] ("
	query=query&"[pcPay_PJ_ID] [int] NULL  DEFAULT (1),"
	query=query&"[pcPay_PJ_MerchantID] [nvarchar] (50) NULL ,"
	query=query&"[pcPay_PJ_MerchantPassword] [nvarchar] (250) NULL ,"
	query=query&"[pcPay_PJ_TransType] [nvarchar] (50) NULL ,"
	query=query&"[pcPay_PJ_cardTypes] [nvarchar] (50) NULL ,"
	query=query&"[pcPay_PJ_CVC] [int] NULL ,"
	query=query&"[pcPay_PJ_TestMode] [int] NULL DEFAULT (0) "
	query=query&");"
	set rs=server.CreateObject("ADODB.RecordSet")
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		TrapSQLError("pcPay_PayJunction")
	else
		query="INSERT INTO pcPay_PayJunction (pcPay_PJ_ID, pcPay_PJ_CVC, pcPay_PJ_TestMode) VALUES (1,0,1);"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp.execute(query)
	end if
...
ELSE
	if request("action")="access" then
		...
		'**** Create table pcPay_PayJunction ***************************************
		query="CREATE TABLE pcPay_PayJunction ("
		query=query&"pcPay_PJ_ID INTEGER NOT NULL DEFAULT 1 ,"
		query=query&"pcPay_PJ_MerchantPassword VarChar(50) NULL ,"
		query=query&"pcPay_PJ_MerchantID VarChar(250) NULL ,"
		query=query&"pcPay_PJ_TransType varchar(50) NULL ,"
		query=query&"pcPay_PJ_cardTypes VarChar(50) NULL ,"
		query=query&"pcPay_PJ_CVC INTEGER NOT NULL DEFAULT 0 ,"
		query=query&"pcPay_PJ_TestMode INTEGER NOT NULL DEFAULT 0 "
		query=query&");"
		
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp1.execute(query)
		if err.number <> 0 then
			TrapError("pcPay_PayJunction")
		else
			query="INSERT INTO pcPay_PayJunction (pcPay_PJ_ID, pcPay_PJ_CVC, pcPay_PJ_TestMode) VALUES (1,0,1);"
			set rs=server.CreateObject("ADODB.RecordSet")
			set rs=conntemp1.execute(query)
...
<form action="upddbPayJunction.asp" method="get" name="form1" class="pcForms">
<%
if mode="complete" then 
	response.redirect "AddModRTPayment.asp?gwchoice=64"
	response.end
else %>
	<table class="pcCPcontent" style="width:600px;" align="center">
		...
		<tr>
			<td>
				...
				<span style="font-weight: bold">You must update your database before you can add PayJunction Gateway.<u></u></span><br>
				In order to activate PayJunction real-time payment gateway you will need to update your ProductCart database to add the required table.</div>				<p><strong><br>
				    <br>
			...
			<% if scDB="Access" then %>
					<tr>
						<td align="center">
						  <input type="button" name="access" value="Update Your ProductCart MS Access Database" onClick="location='upddbPayJunction.asp?action=access';" class="submit2">
						</td>
					</tr>
			<% 
				else
			...
			<% end if %>
				<tr>
					...
					<% if request.querystring("mode")="1" OR request.querystring("mode")="3" then %>
						...
						<% else %>
					  <input type="button" name="access2" value="Update Your ProductCart MS SQL Database" onClick="location='upddbPayJunction.asp?action=sql';" class="submit2">
						<% end if %>
					</td>
			</tr>
			<% end if %>
