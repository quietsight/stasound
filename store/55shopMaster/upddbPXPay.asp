...
<% '--Updater File-- %>
<% pageTitle = "PX Pay Gateway - Database Update" %>
<% Section = "" %>
<%PmAdmin=19%>
...
IF request("action")="sql" then
	if request.querystring("hmode")="2" then
		...
		if SSIP="" or UID="" or PWD="" then
			response.redirect "upddbPXPay.asp?mode=3"
			response.End
		end if
		...
		if instr(ucase(scDSN),"DSN=") then
			response.redirect "upddbPXPay.asp?mode=1"
			response.End
		end if
		call openDb()
	end if
	...
	ErrStr=""		
		...
		'// Create table Pay_DowCom
		query="CREATE TABLE [dbo].[pcPay_PxPay] ("
		query=query&"[pcPay_PxPay_ID] [INT] IDENTITY (1, 1) PRIMARY KEY CLUSTERED NOT NULL ,"
		query=query&"[pcPay_PxPay_PxPayUserId] [nvarchar] (100) NULL ,"
		query=query&"[pcPay_PxPay_PxPayTestUserId] [nvarchar] (100) NULL ,"
		query=query&"[pcPay_PxPay_PxPayKey] [nvarchar] (100) NULL ,"
		query=query&"[pcPay_PxPay_TxnType] [nvarchar] (100) NULL ,"
		query=query&"[pcPay_PxPay_TestMode] [int]  DEFAULT(0) NULL,"
		query=query&"[pcPay_PxPay_CurrencyInput] [nvarchar] (10) NULL "
		...
		if err.number <> 0 then
			...
			query ="insert into pcPay_PxPay(pcPay_PxPay_PxPayUserId,pcPay_PxPay_PxPayTestUserId,pcPay_PxPay_PxPayKey,pcPay_PxPay_TxnType) VALUES('','','','')"
			set rs=server.CreateObject("ADODB.RecordSet")
			set rs=conntemp.execute(query)
		end if
...
ELSE
	if request("action")="access" then
		...
		'**** Create table pcPay_PX Pay ***************************************
		query="CREATE TABLE pcPay_PxPay ("
		query=query&"pcPay_PxPay_ID Counter NOT NULL PRIMARY KEY UNIQUE ,"
		query=query&"pcPay_PxPay_PxPayUserId VarChar(100) NULL ,"
		query=query&"pcPay_PxPay_PxPayTestUserId VarChar(100) NULL ,"
		query=query&"pcPay_PxPay_PxPayKey VarChar(100) NULL ,"		
		query=query&"pcPay_PxPay_TxnType VarChar(100) NULL ,"		
		query=query&"pcPay_PxPay_TestMode INTEGER NULL DEFAULT '0',"
		query=query&"pcPay_PxPay_CurrencyInput VarChar(10) NULL "
		query=query&");"
		set rs=server.CreateObject("ADODB.RecordSet")
...
<form action="upddbPXPay.asp" method="get" name="form1" class="pcForms">
<%
if mode="complete" then 
	response.redirect "AddModRTPayment.asp?gwchoice=12"
	response.end
else 
%>
	<table class="pcCPcontent" style="width:600px;" align="center">
		...
		<tr>
			<td>
				...
				<span style="font-weight: bold">You must update your database before you can add PX Pay Gateway.<u></u></span><br>
				In order to activate PX Pay real-time payment gateway you will need to update your ProductCart database to add the required table.</div>				<p><strong><br>
				    <br>
			...
			<% if scDB="Access" then %>
					<tr>
						<td align="center">
						  <input type="button" name="access" value="Update Your ProductCart MS Access Database" onClick="location='upddbPXPay.asp?action=access';" class="submit2">
						</td>
					</tr>
			<% 
				else
			...
			<% end if %>
				<tr>
					...
					<% if request.querystring("mode")="1" OR request.querystring("mode")="3" then %>
						...
						<% else %>
					  <input type="button" name="access2" value="Update Your ProductCart MS SQL Database" onClick="location='upddbPXPay.asp?action=sql';" class="submit2">
						<% end if %>
					</td>
			</tr>
			<% end if %>
