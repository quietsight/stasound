...
call opendb()
session("CP_NW_ListID")=getUserInput(request("MailUpListID"),0)
...
CustCountryCode=getUserInput(request("CustCountryCode"),0)
CustStateCode=getUserInput(request("CustStateCode"),0)
CustProvince=getUserInput(request("CustProvince"),0)
CustZipCode=getUserInput(request("CustZipCode"),0)
CustStartDate=getUserInput(request("CustStartDate"),0)
CustEndDate=getUserInput(request("CustEndDate"),0)
CustTOType=getUserInput(request("CustTOType"),0)
Select Case CustTOType
	Case 0: CustTOType=">"
	Case 1: CustTOType="<"
	Case 2: CustTOType=">="
	Case 3: CustTOType="<="
End Select
CustTotalAmount=getUserInput(request("CustTotalAmount"),0)
...
function checkNoOrderCustomer(COptedIn,CCustType)
	...
	if (COptedIn<>"") and (COptedIn<>"0") then
		...
		if COptedIn="1" then
			myTemp4=myTemp4 & " customers.RecvNews=1"
		else
			if COptedIn="2" then
				myTemp4=myTemp4 & " customers.RecvNews=0"
			end if
		end if
	end if
	
	if (CustCountryCode<>"") then
		if myTemp4="" then
			myTemp4=" where "
		else
			myTemp4=myTemp4 & " and "
		end if
		myTemp4=myTemp4 & " customers.countryCode like '" & CustCountryCode & "'"
	end if
	
	if (CustStateCode<>"") then
		if myTemp4="" then
			myTemp4=" where "
		else
			myTemp4=myTemp4 & " and "
		end if
		myTemp4=myTemp4 & " customers.stateCode like '" & CustStateCode & "'"
	end if
	
	if (CustProvince<>"") then
		if myTemp4="" then
			myTemp4=" where "
		else
			myTemp4=myTemp4 & " and "
		end if
		myTemp4=myTemp4 & " customers.state like '%" & CustProvince & "%'"
	end if
	...
	if (CustZipCode<>"") then
		...
		myTemp4=myTemp4 & " customers.zip like '%" & CustZipCode & "%'"
	end if
	
	if (CustStartDate<>"") and (IsDate(CustStartDate)) then
		if myTemp4="" then
			myTemp4=" where "
		else
			myTemp4=myTemp4 & " and "
		...
		if scDB="Access" then
			myTemp4=myTemp4 & " customers.pcCust_DateCreated>=#" & CustStartDate & "#"
		else
			myTemp4=myTemp4 & " customers.pcCust_DateCreated>='" & CustStartDate & "'"
		end if
	end if
	
	if (CustEndDate<>"") and (IsDate(CustEndDate)) then
		CustEndDate=CDate(CustEndDate)+1
		if myTemp4="" then
			myTemp4=" where "
		else
			myTemp4=myTemp4 & " and "
		...
		if scDB="Access" then
			myTemp4=myTemp4 & " customers.pcCust_DateCreated<#" & CustEndDate & "#"
		else
			myTemp4=myTemp4 & " customers.pcCust_DateCreated<'" & CustEndDate & "'"
		end if
	end if
	
	if (CustTotalAmount<>"") AND (IsNumeric(CustTotalAmount)) then
		if myTemp4="" then
			myTemp4=" where "
		else
			myTemp4=myTemp4 & " and "
		end if
		myTemp4=myTemp4 & " customers.TotalSales" & CustTOType & CustTotalAmount
	end if
