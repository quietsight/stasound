...
Function gwPPPEdit()
	...
	query="UPDATE payTypes SET pcPayTypes_processOrder=" & pcv_processOrder & ",pcPayTypes_setPayStatus=" & pcv_setPayStatus & ",paymentDesc='"&PayPalName&"',sslUrl='gwPayPal.asp',priceToAdd="& priceToAdd &" , percentageToAdd="& percentageToAdd &", paymentNickName='"&paymentNickName&"',pcPayTypes_ppab=0 WHERE gwCode=46"
	set rs=Server.CreateObject("ADODB.Recordset")     
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		strErrorDescription=err.description
...
Function gwPPP()
	...
	query="INSERT INTO payTypes (pcPayTypes_processOrder,pcPayTypes_setPayStatus,paymentDesc, sslURL, active, Cbtob, quantityFrom, quantityUntil, weightFrom, weightUntil, priceFrom, priceUntil,ssl,priceToAdd, percentageToAdd,gwCode,paymentNickName, pcPayTypes_ppab) VALUES (" & pcv_processOrder & "," & pcv_setPayStatus & ",'PayPal','gwPayPal.asp',-1,0,0,9999,0,9999,0,9999,-1,"& priceToAdd &","& percentageToAdd &",46,'"&paymentNickName&"',0)"
	set rs=Server.CreateObject("ADODB.Recordset")     
	set rs=connTemp.execute(query)
	if err.number<>0 then
		call LogErrorToDatabase()
...
<% if request("gwchoice")="46" then
	...
	if request("mode")="Edit" then
		...
		query= "SELECT pcPayTypes_processOrder, pcPayTypes_setPayStatus, priceToAdd, percentageToAdd, paymentNickName FROM payTypes WHERE gwCode=46"
		set rs=Server.CreateObject("ADODB.Recordset")     
		set rs=conntemp.execute(query)
		if rs.eof then
			paymentNickName="Credit Card"
    ...
    <% else
		...
		call opendb()
		query= "SELECT idPayment, gwCode FROM payTypes WHERE gwCode=3 OR gwCode=53 OR gwCode=80"
		set rs=Server.CreateObject("ADODB.Recordset")     
		set rs=conntemp.execute(query)
		if NOT rs.eof then
        	...
        	<div class="pcCPmessage">
        	  <p>You currently have <strong><%=pcConflictDesc%></strong> active for this store. In order to use <strong>PayPal Payments Pro</strong> you will need to first disable <strong><%=pcConflictDesc%></strong>.<br />
        	    <br />
        	  </p>
        	  <p><a href="pcConfigurePayment.asp?mode=Del&id=<%=pcConflictIdPayment%>&gwchoice=<%=pcConflictID%>&activate=46">Disable <%=pcConflictDesc%> and enable PayPal Payments Pro</a></p>
        	  <br />
...
<% if pcConflictIdPayment = 0 then %>
	<input type="hidden" name="addGw" value="46">
    <div class="pcCPmessageSuccess">
        <% if request("mode")="Edit" then %>
            <p><strong>You're editing </strong><strong>PayPal Payments Pro</strong>
            <br />
            <br />
            <p><strong><a href="pcPaymentSelection.asp">Change Payment Option</a></strong></p>
            <br /></p>
        <% else %>
            <p><strong>You've selected PayPal Payments Pro</strong><br />
            ...
            <td>
                <div id="CollapsiblePanel1" class="CollapsiblePanel">
                    <div class="CollapsiblePanelTab1">
                        <table width="100%">
                          <tr>
                            <td width="580" class="pcPanelTitle1"><strong>Step 1: Configure Account - PayPal Payments Pro (US)</strong></td>
                          </tr>
                        </table>
                    </div>
                        ...
                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
								...
								<tr>
								  <td colspan="2" valign="top">
                                    <p>You must have a PayPal business account to use PayPal Payments Pro. If you don't have an account, <a href="#">sign up for one now</a>.</p><br />
<p>Enter your PayPal Account Information
We need this information to work with PayPal so
that payments can be sent to your account.</p><br />

<p><a href="https://www.paypal.com/us/cgi-bin/webscr?cmd=_get-api-signature&amp;generic-flow=true" target="_blank">Get your API Credentials.</a></p><br /></td>
						  </tr>
								<tr> 
								...
								<tr> 
									...
									<td><b>Enable Test Mode </b>(Credit cards will not be charged) <a href="JavaScript:win('helpOnline.asp?ref=805')"><img src="images/pcv3_infoIcon.gif" alt="More information on this feature" /></a>									  </td>
								</tr>
								<tr> 
									<td> <div align="right">  
									<input name="pcPay_PayPal_CVC" type="checkbox" class="clearBorder" value="1" <% if pcPay_PayPal_CVC=1 then%>checked<% end if %>>
										</div></td>
									<td><b>Enable Credit Card Security Code</b></td>
								</tr>
                            <tr>
                                <td>&nbsp;</td>
