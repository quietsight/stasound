...
If InValidImage>0 then
	...
	if pcTmpErrSize = 1 then
		...
		response.write "An Error occurred while attempting to upload your images: "&err.description&"<br><br>This is probably due to a default configuration option in IIS, which only allows the entity size in a POST request to be 200,000 bytes (~200KB).<br><br>You have to change this setting if you want to upload multiple images files or large image files through the ProductCart application (otherwise you can just FTP the file to the same location: e.g. the &quot;pc/catalog&quot; folder).<br><br>To change this setting:<br><br>- Open IIS Manager<br>- Navigate the tree to your application<br>- Double click the &quot;ASP&quot; icon in the main panel<br>- Expand the &quot;Limits&quot; category<br>- Modify the &quot;Maximum Requesting Entity Body Limit&quot; to a larger value<br> "
	else
		response.redirect "ImageUploada.asp?msg="&Server.URLEncode(InValidImage&" of your "&ImageCnt&" images were not in a valid image format and could not be uploaded to the server.")
	end if
	'//END DETAILED MSG
Else
	if ImageCnt>0 then
		response.redirect "ImageUploada.asp?s=1&msg="&Server.URLEncode("Your images were successfully uploaded.")
	else
		response.redirect "ImageUploada.asp?msg="&Server.URLEncode("You need to supply at least one file to upload.")
	end if
end if
%>
