...
<% '--Updater File-- %>
<% pageTitle = "eProcessing Network Gateway -   Database Update" %>
<% Section="paymntOpt" %>
...
IF request("action")="sql" then
	if request.querystring("hmode")="2" then
		...
		if SSIP="" or UID="" or PWD="" then
			response.redirect "upddbEPN.asp?mode=3"
			response.End
		end if
		...
		if instr(ucase(scDSN),"DSN=") then
			response.redirect "upddbEPN.asp?mode=1"
			response.End
		end if
		call openDb()
	end if
	...
	query="ALTER TABLE [pcPay_EPN] ADD [pcPay_EPN_RestrictKey] [nvarchar] (255) NULL"
	...
	if err.number <> 0 then
		if instr(ucase(Err.Description),"COLUMN NAMES IN EACH TABLE MUST BE UNIQUE") then
			...
			ErrStr=ErrStr&"Unable to update TABLE pcPay_EPN - Error: "&Err.Description&"<BR>"
			err.number=0
			iCnt=iCnt+1
		end if
	end if
...
ELSE
	if request("action")="access" then
		...
		query="ALTER TABLE pcPay_EPN ADD pcPay_EPN_RestrictKey varchar(255) NULL;"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=connTemp1.execute(query)
		if err.number <> 0 then
			call TrapError("pcPay_EPN")
		end if
...
<form action="upddbEPN.asp" method="get" name="form1" class="pcForms">
<% 
'mode=Edit&id=219&gwchoice=11
if strMode="complete" then
	response.redirect "AddModRTPayment.asp?mode="&tmp_mode&"&id="&tmp_id&"&gwchoice=42"
	response.end
else %>
	...
	<table class="pcCPcontent" style="width:600px;" align="center">
		...
		<tr>
			...
			<% if scDB="Access" then %>
					<tr>
						<td align="center">
						  <input type="button" name="access" value="Update Your ProductCart MS Access Database" onClick="location='upddbEPN.asp?action=access&mode=<%=tmp_mode%>&id=<%=tmp_id%>';" class="submit2">
						</td>
					</tr>
			<% 
				else
			...
			<% end if %>
				<tr>
					...
					<% if request.querystring("mode")="1" OR request.querystring("mode")="3" then %>
						...
						<% else %>
					  <input type="button" name="access2" value="Update Your ProductCart MS SQL Database" onClick="location='upddbEPN.asp?action=sql&mode=<%=tmp_mode%>&id=<%=tmp_id%>';" class="submit2">
						<% end if %>
					</td>
			</tr>
			<% end if %>
