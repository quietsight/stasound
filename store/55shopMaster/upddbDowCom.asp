...
IF request("action")="sql" then
	...
	if err.number <> 0 then
		...
		query="INSERT INTO pcPay_DowCom (pcPay_Dow_MerchantID, pcPay_Dow_MerchantPassword, pcPay_Dow_TransType, pcPay_Dow_CardTypes) VALUES ('testearlyimpact ','early123','auth','V, M');"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp.execute(query)
	end if
...
ELSE
	if request("action")="access" then
		...
		'**** Create table pcPay_DowCom ***************************************
		query="CREATE TABLE pcPay_DowCom ("
		query=query&"pcPay_Dow_ID INTEGER NOT NULL DEFAULT 1 ,"
		query=query&"pcPay_Dow_MerchantID VarChar(100) NULL ,"
		query=query&"pcPay_Dow_MerchantPassword VarChar(250) NULL ,"
		query=query&"pcPay_Dow_TransType VarChar(20) NULL ,"
		query=query&"pcPay_Dow_CardTypes VarChar(250) NULL ,"
		query=query&"pcPay_Dow_CVC INTEGER NOT NULL DEFAULT 0 ,"
		query=query&"pcPay_Dow_eCheck INTEGER NOT NULL DEFAULT 0 ,"
		query=query&"pcPay_Dow_eCheckPending INTEGER NOT NULL DEFAULT 0 ,"
		query=query&"pcPay_Dow_TestMode INTEGER NOT NULL DEFAULT 0 "
		query=query&");"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp1.execute(query)
		if err.number <> 0 then
			TrapError("pcPay_DowCom")
		else
			query="INSERT INTO pcPay_DowCom (pcPay_Dow_MerchantID, pcPay_Dow_MerchantPassword, pcPay_Dow_TransType, pcPay_Dow_CardTypes) VALUES ('testearlyimpact ','early123','auth','V, M');"
			set rs=server.CreateObject("ADODB.RecordSet")
			set rs=conntemp1.execute(query)
...
if mode="complete" then 
	response.redirect "AddModRTPayment.asp?gwchoice=60"
	response.end
else %>
	<table class="pcCPcontent" style="width:600px;" align="center">
		...
		<tr>
			...
			<% if scDB="Access" then %>
					<tr>
						<td align="center">
						  <input type="button" name="access" value="Update Your ProductCart MS Access Database" onClick="location='upddbDowCom.asp?action=access';" class="submit2">
						</td>
					</tr>
			<% 
				else
			...
			<% end if %>
				<tr>
					...
					<% if request.querystring("mode")="1" OR request.querystring("mode")="3" then %>
						...
						<% else %>
					  <input type="button" name="access2" value="Update Your ProductCart MS SQL Database" onClick="location='upddbDowCom.asp?action=sql';" class="submit2">
						<% end if %>
					</td>
			</tr>
			<% end if %>
