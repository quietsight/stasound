...
If scPPLogging = "1" Then
	if PPD="1" then
		pcStrLogName=Server.Mappath ("/"&scPcFolder&"/includes/PPACaptureLog.txt")
	else
		pcStrLogName=Server.Mappath ("../includes/PPACaptureLog.txt")
	end if
...
if pcPay_PayPalAd_Vendor&""="" then
	pcPay_PayPalAd_Vendor = pcPay_PayPalAd_MerchantLogin
end if		
	...
	IF request.Form("checkOrd"&r)="YES" THEN
		...
		if cdbl(pAmount)>0 then		
			...
			if not IsEmpty(pidAuthOrder) then
				...
				nvpstr = nvpstr &"&PARTNER="& pcPay_PayPalAd_Partner
				nvpstr = nvpstr &"&USER="& pcPay_PayPalAd_User
				nvpstr = nvpstr &"&VENDOR="& pcPay_PayPalAd_Vendor
				nvpstr = nvpstr &"&PWD="& pcPay_PayPalAd_Password
				...
				If scPPLogging = "1" Then
					...
					OutputFile.WriteLine "Request from ProductCart to PayPal for PayPal Advanced Payments Capture"
				...
				'//PAYPAL LOGGING START
				If scPPLogging = "1" Then
					OutputFile.WriteLine "Response from PayPal to ProductCart for PayPal Advanced Payments Capture"
