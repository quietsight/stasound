...
<html>
<head>
	<title>Online Sales Report - Sales by Category</title>
...
if not validNum(pIDCategory) or pIDCategory="0" then
	response.redirect "msgb.asp?msg=" & Server.URLEncode("You did not select a valid category.")
end if
...
<% Else %>
	<tr> 
		<td colspan="10"><h2>Total Sales recorded from: <%=strTDateVar%> to: <%=strTDateVar2%> <%if CatName<>"" then%>&nbsp;for <%=CatName%><%end if%></h2></td>
	</tr>
	<tr> 
...
do until rs.EOF
	...
	query="SELECT Products.idproduct,ProductsOrdered.quantity,(ProductsOrdered.unitPrice*ProductsOrdered.quantity) AS SubPTotal,Products.Description  FROM (ProductsOrdered INNER JOIN Products ON ProductsOrdered.idProduct=Products.idProduct) INNER JOIN Categories_Products ON ProductsOrdered.idProduct=Categories_Products.idProduct WHERE Categories_Products.idcategory=" & pIDCategory & " and ProductsOrdered.idOrder=" &  pc_idorder & ";"
