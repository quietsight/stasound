...
<% '--Updater File-- %>
<% pageTitle = "Omega Gateway - Database Update" %>
<% Section = "" %>
<%PmAdmin=19%>
...
IF request("action")="sql" then
	if request.querystring("hmode")="2" then
		...
		if SSIP="" or UID="" or PWD="" then
			response.redirect "upddbTXP.asp?mode=3"
			response.End
		end if
		...
		if instr(ucase(scDSN),"DSN=") then
			response.redirect "upddbTXP.asp?mode=1"
			response.End
		end if
		call openDb()
	end if
	...
	'**** Create table pcPay_TransFirstXP ***************************************
	query="CREATE TABLE [dbo].[pcPay_TransFirstXP] ("
	query=query&"[pcPay_TransFirstXPID] [int] NULL  DEFAULT (1),"
	query=query&"[pcPay_TXPGatewayID] [nvarchar] (100) NULL ,"
	query=query&"[pcPay_TXPRegistrationKey] [nvarchar] (250) NULL ,"
	query=query&"[pcPay_TXPCardTypes] [nvarchar] (200) NULL ,"
	query=query&"[pcPay_TXPTransType] [nvarchar] (20) NULL ,"
	query=query&"[pcPay_TXPReqCardCode] [int] NULL DEFAULT(0) ,"
	query=query&"[pcPay_TXPTestMode] [int] NULL DEFAULT(0)"
	query=query&");"
	set rs=server.CreateObject("ADODB.RecordSet")
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		TrapSQLError("pcPay_TransFirstXP")
	else
		query="INSERT INTO pcPay_TransFirstXP (pcPay_TransFirstXPID, pcPay_TXPReqCardCode, pcPay_TXPTestMode) VALUES (1,0,1);"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp.execute(query)
	end if
...
ELSE
	if request("action")="access" then
		...
		'**** Create table pcPay_TransFirstXP ***************************************
		query="CREATE TABLE pcPay_TransFirstXP ("
		query=query&"pcPay_TransFirstXPID INTEGER NOT NULL DEFAULT 1 ,"
		query=query&"pcPay_TXPGatewayID VarChar(100) NULL ,"
		query=query&"pcPay_TXPRegistrationKey VarChar(250) NULL ,"
		query=query&"pcPay_TXPCardTypes VarChar(250) NULL ,"
		query=query&"pcPay_TXPTransType VarChar(3) NULL ,"
		query=query&"pcPay_TXPReqCardCode INTEGER NOT NULL DEFAULT 0 ,"
		query=query&"pcPay_TXPTestMode INTEGER NOT NULL DEFAULT 0 "
		query=query&");"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp1.execute(query)
		if err.number <> 0 then
			TrapError("pcPay_TransFirstXP")
		else
			query="INSERT INTO pcPay_TransFirstXP (pcPay_TransFirstXPID, pcPay_TXPReqCardCode, pcPay_TXPTestMode) VALUES (1,0,1);"
			set rs=server.CreateObject("ADODB.RecordSet")
			set rs=conntemp1.execute(query)
...
<form action="upddbTXP.asp" method="get" name="form1" class="pcForms">
<%
if mode="complete" then 
	response.redirect "AddModRTPayment.asp?gwchoice=70"
	response.end
else %>
	<table class="pcCPcontent" style="width:600px;" align="center">
		...
		<tr>
			<td>
				...
				<span style="font-weight: bold">You must update your database before you can add TransFirst Express Gateway.<u></u></span><br>
				In order to activate TransFirst Express real-time payment gateway you will need to update your ProductCart database to add the required table.</div>				<p><strong><br>
				    <br>
			...
			<% if scDB="Access" then %>
					<tr>
						<td align="center">
						  <input type="button" name="access" value="Update Your ProductCart MS Access Database" onClick="location='upddbTXP.asp?action=access';" class="submit2">
						</td>
					</tr>
			<% 
				else
			...
			<% end if %>
				<tr>
					...
					<% if request.querystring("mode")="1" OR request.querystring("mode")="3" then %>
						...
						<% else %>
					  <input type="button" name="access2" value="Update Your ProductCart MS SQL Database" onClick="location='upddbTXP.asp?action=sql';" class="submit2">
						<% end if %>
					</td>
			</tr>
			<% end if %>
