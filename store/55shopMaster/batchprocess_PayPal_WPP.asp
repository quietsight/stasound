...
If scPPLogging = "1" Then
	if PPD="1" then
		pcStrLogName=Server.Mappath ("/"&scPcFolder&"/includes/PPPCaptureLog.txt")
	else
		pcStrLogName=Server.Mappath ("../includes/PPPCaptureLog.txt")
	end if
	...
	IF request.Form("checkOrd"&r)="YES" THEN
		...
		if cdbl(pAmount)>0 then		
			...
			if not IsEmpty(pidAuthOrder) then
				...
				'---------------------------------------------------------------------------
				nvpstr="" '// clear 
				objPayPalClass.AddNVP "AUTHORIZATIONID", pidAuthOrder
				...
				If scPPLogging = "1" Then
					...
					OutputFile.WriteLine "Request from ProductCart to PayPal for PayPal Payments Pro Capture"
				...
				'//PAYPAL LOGGING START
				If scPPLogging = "1" Then
					OutputFile.WriteLine "Response from PayPal to ProductCart for PayPal Payments Pro Capture"
