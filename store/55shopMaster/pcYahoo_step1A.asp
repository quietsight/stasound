...
if pcv_prdlist<>"" then
	session("cp_exportYahoo_prdlist")=pcv_prdlist
	if pcv_prdlist="ALL1" then
		session("cp_exportYahoo_prdlist")="ALL"
		session("cp_exportYahoo_pagecurrent")=""
	else
		session("cp_exportYahoo_pagecurrent")=request("pagecurrent")
	end if
	response.redirect "pcYahoo_step2.asp"
end if
%>
	...
	<td colspan="2">
		...
		<tr>
			<td>
				...
				src_FormTitle2="Export to Yahoo! Search Marketing Wizard - Locate products"
				...
				src_DisplayType=1
				src_ShowLinks=0
				src_FromPage="pcYahoo_step1A.asp"
				src_ToPage="pcYahoo_step1A.asp"
				src_Button1=" Search "
