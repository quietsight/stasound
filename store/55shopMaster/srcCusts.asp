...
form_key6=getUserInput(request("key6"),0)
form_key7=getUserInput(request("key7"),0)
form_key8=getUserInput(request("key8"),0)
form_key9=getUserInput(request("key9"),0)
form_key10=getUserInput(request("key10"),0)
form_key11=getUserInput(request("key11"),0)
...
<% ELSE%>
	...
	function runXML()
	{
		...
		myConn.connect("xml_srcCusts.asp", "GET", GetAllValues(document.ajaxSearch), fnWhenDone);
	}
	...
	<input type="hidden" name="key6" value="<%=form_key6%>">
	<input type="hidden" name="key7" value="<%=form_key7%>">
	<input type="hidden" name="key8" value="<%=form_key8%>">
	<input type="hidden" name="key9" value="<%=form_key9%>">
	<input type="hidden" name="key10" value="<%=form_key10%>">
	<input type="hidden" name="key11" value="<%=form_key11%>">
	...
	<input type="hidden" name="custlist" value="">
	</form>
...
<p style="text-align: center;">
	<%if (src_ToPage<>"") and (pcv_HaveResults="1") and (src_DisplayType>"0") then
		...
		<input type="button" value="<%=src_Button2%>" onClick="javascript:if (savelist=='xml,') {alert('Please select a customer before clicking on this button');} else {location.href='<%=src_ToPage%>custlist='+getcustlist(savelist);}" class="ibtnGrey">
	<%end if%>
