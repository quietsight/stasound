...
dim f, query, conntemp, rstemp, rstemp1,TopRecord(100)
...
if RecordError=false then 'STEP 1
	...
	IF session("append")="1" then
		...
		ELSE 'Do not have existing Category
			...
			if catName<>"" then
				ErrorsReport=ErrorsReport & "Record " & TotalXLSlines & ": The Category Name: '" & catName & "' is not in the database." & vbcrlf
			else
				ErrorsReport=ErrorsReport & "Record " & TotalXLSlines & ": The Category ID: " & catID & " is not in the database." & vbcrlf
			end if
			RecordError=true
		END IF
	...
	ELSE 'Append=0
		...
		if testCat=1 then
			...
			set rs=connTemp.execute(query)
			if not rs.eof then
				ErrorsReport=ErrorsReport & "Record " & TotalXLSlines & ": The Category Name: '" & catName & "' could not be imported because it already exists in the same parent category." & vbcrlf
				RecordError=true
			end if
			set rs=nothing
		end if
