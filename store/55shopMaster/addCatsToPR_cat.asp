...
if request("action")="add" then
	...
	For i=1 to Count
		...
		if pidcode<>"0" then
			query="INSERT INTO pcCPFCategories (pcCatPro_ID,idcategory,pcCPFCats_IncSubCats) values (" & pidcode & "," & IDPro & "," & IDSub & ")"
...
<form name="hForm" method="post" action="addCatsToPR_cat.asp?action=add">
<tr class="normal">
...
Dim query,rs,i,pIDCat,pName,myTest,tmpPath
	...
	if clng(pcArr(2,i))=clng(idparent) then
		pIDCat=pcArr(0,i)
		pName=pcArr(1,i)
		query="select idcategory,pcCPFCats_IncSubCats from pcCPFCategories where pcCatPro_ID=" & pidcode & " and idcategory=" & pIDCat
		set rs=connTemp.execute(query)
		myTest=0
		if rs.eof then
			myTest=1
		else
			if rs("pcCPFCats_IncSubCats")="0" then
				myTest=1
			end if
		end if
		if rs.eof then
