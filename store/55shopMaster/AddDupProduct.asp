...
pcv_ProductType=lcase(trim(request.Querystring("prdType")))
	...
	if pcv_ProductType="" or (pcv_ProductType<>"std" and pcv_ProductType<>"bto" and pcv_ProductType<>"item") then
		...
		response.Redirect "FindDupProductType.asp?id="&pIdProduct&"&tab="&tab
	end if
...
dim ACat
ACat=""
query="select IDCategory from Categories_Products where IDProduct=" & request("idproduct")
...
if rs.eof then
	set rs=nothing
	call closedb()
	response.redirect "addProduct.asp?prdType=" & pcv_ProductType
end if
...
pageTitle="Clone Product: <strong>" & pDescription & "</strong>"
...
<script type="text/javascript">
	<!--
	var dsCategoriesList = new Spry.Data.XMLDataSet("pcSpryCategoriesXML.asp?idRootCategory=<%=pcv_IdRootCategory%>&strCats=<%=cat_SelectedItems%>", "categories/category",{sortOnLoad:"categoryDesc",sortOrderOnLoad:"ascending",useCache:false});
...
if pcSCID>"0" and scDB="SQL" then
	...
	if not rsS.eof then
		...
		<tr> 
		<td colspan="2">
		<div class="pcCPmessageInfo">You are cloning an "On Sale" product. This product is currently included in the running Sale: <a href="javascript:winSale('sm_showdetails.asp?id=<%=pcSCID%>')"><%=pcSCName%></a><br>
		Please remember the product price is not the normal price.</div>
		</td>
		</tr>
		<%
	end if
...
'SM-END%>
        <tr> 
            <td colspan="2">
				<div class="cpOtherLinks" style="float: right; margin: 0px;">
...
<form method="post" name="hForm" action="addProductB.asp" onSubmit="return Form1_Validator(this)" class="pcForms">
		...
		<table class="pcCPcontent">
            ...
            <%
			tmp_Array=split(cat_SelectedItems,",")
			tmp_Cnt=0
			tmp1=""
			tmp2=""
			For l=lbound(tmp_Array) to ubound(tmp_Array)
				if trim(tmp_Array(l))<>"" then
					if tmp_Cnt=0 then
						tmp1=tmp1&"'{idCategory}' == '"&tmp_Array(l)&"'"
						tmp2=tmp2&"'{idCategory}' != '"&tmp_Array(l)&"'"
					else
						tmp1=tmp1&" || '{idCategory}' == '"&tmp_Array(l)&"'"
						tmp2=tmp2&" && '{idCategory}' != '"&tmp_Array(l)&"'"
					end if
					...
					<div spry:region="dsCategoriesList" class="SpryHiddenRegion">
						<div spry:state="ready">
							<select name="idCategory1" size="10" multiple spry:repeatchildren="dsCategoriesList" style="width: 600px;">
                                <option spry:if="<%=tmp1%>" value="{idCategory}" selected="selected">{categoryDesc} {pcCats_BreadCrumbs}</option>
                                <option spry:if="<%=tmp2%>" value="{idCategory}">{categoryDesc} {pcCats_BreadCrumbs}</option>
							</select>
						</div>
						<div spry:state="loading">Loading...</div>
					</div>   
		...
		%>
			<ul class="TabbedPanelsTabGroup">
				...
				<li class="TabbedPanelsTabButtons" tabindex="1300">
						...
						<input type="hidden" name="AddType" value="19">
						<input type="hidden" name="PreProd" value="<%=request("idproduct")%>">
						<input type="hidden" name="prdType" value="<%=pcv_ProductType%>">
						<input type="submit" name="Submit" value="Add Product" class="submit2" disabled>
				</li>
			</ul>
			...
			%>
				<div class="TabbedPanelsContent">
					...
					<table class="pcCPcontent">
						...
						<% if pcv_ProductType<>"item" then ' Hide if it's a BTO Only Item
							query="SELECT pcProdImage_Url,pcProdImage_LargeUrl,pcProdImage_Order FROM pcProductsImages WHERE idProduct=" & pIdProduct & ";"
    						set rstemp=server.CreateObject("ADODB.RecordSet")
						    set rstemp=conntemp.execute(query)
							if not rstemp.eof then
								response.write "<tr><td colspan='2'><i>Additional Images will be added to new product after cloning.</i></td></tr>"
							end if
						end if ' Hide if it's a BTO Only Item %>
								...
								if not rs.eof then
									...
									<table class="pcCPcontent" style="width:auto;">
										...
										<tr>
											...
											<td>
												<input type="text" value="" name="newvalue" size="30">&nbsp;&nbsp;<%=dictLanguageCP.Item(Session("language")&"_cpInstPrd_95")%> <input type="text" value="0" name="neworder" size="3">
...
function pcf_checkCatList()
{
	try
	{
	if (document.hForm.idCategory1.value=="{idCategory}")
	{
		setTimeout("pcf_checkCatList()",500)
	}
	else
	{
		document.hForm.Submit.disabled=false;
	}
	}
	catch(e)
	{
...
pcf_WaitOnBody(pcf_checkCatList);
</script>
