...
mySQL="select * from pcComments where pcComm_IDFeedback=" & lngIDFeedback & " and pcComm_IDParent=0 and pcComm_IDOrder=" & lngIDOrder & " and pcComm_IDUser=" & session("IDCustomer")
set rstemp=connTemp.execute(mySQL)
 
if rstemp.eof then
	call closedb()
	response.redirect "userviewfeedback.asp?IDOrder=" & scpre+clng(lngIDOrder) & "&IDFeedback=" & lngIDFeedback & "&r=1&msg=This feedback was not found or you don't have permission to delete it."
end if
...
response.redirect "userviewallposts.asp?IDOrder=" & scpre+clng(lngIDOrder)
%>