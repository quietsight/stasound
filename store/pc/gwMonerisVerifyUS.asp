...
transactionKey = request("verify_key")

'Response.write strHostURL & stext & "<BR><BR>" & transactionKey
'Response.end 
...
if result="0" then
	...
	response.redirect "msgb.asp?message="&server.URLEncode("<b>Errors&nbsp;</b>:&nbsp;"&message&"<br><br><a href="""&tempURL&"?psslurl=gwMonerisUS.asp&idCustomer="&session("idCustomer")&"&idOrder="&session("GWOrderId")&"""><img src="""&rslayout("back")&""" border=0></a>")
	response.end
end if
...
'Build XML string
		...
		stext=stext & "&verify_key=" & server.URLEncode(transactionKey)
