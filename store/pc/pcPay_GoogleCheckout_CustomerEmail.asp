...
query="SELECT products.sku, products.idproduct, products.description, products.price, quantity, unitPrice, xfdetails"
'BTO ADDON-S
if scBTO=1 then
...
Do While Not rsOrderDetails.EOF
	...
	xfdetails=replace(rsOrderDetails("xfdetails"),"&lt;BR&gt;"," - ")
		...
		if pIdConfigSession<>"0" then
			...
			for i=lbound(ArrProduct) to (UBound(ArrProduct)-1)
				...
				if rsObj1("displayQF")=True then
					dispStr = pcv_strBtoItemCat &": "& pcv_strBtoItemName & " - QTY: " & ArrQuantity(i)
				else
				...
				if (CDbl(ArrValue(i))<>0) or (((ArrQuantity(i)-1)*pQty>0) and (ArrPrice(i)>0)) then
	...
	customerEmail=customerEmail & FixedField(50, "L", dictLanguage.Item(Session("language")&"_sendMail_18"))
	customerEmail=customerEmail & FixedField(10, "R", scCurSign & money(pPrice1))
	customerEmail=customerEmail & FixedField(10, "R", scCurSign & money(pExtendedPrice1))
	customerEmail=customerEmail & vbCrLf
...
CustomerEmail=customerEmail & dictLanguage.Item(Session("language")&"_sendMail_46") & vbcrlf
...
if (scAllowSeparate="1") and (pcv_BackOrderStr<>"") and (pcv_CustomerReceived=0) and (pcv_SubmitType=3) then
