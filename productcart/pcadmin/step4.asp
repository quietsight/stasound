...
dim CSVRecord(300),f, query, conntemp, rstemp, rstemp1,TopRecord(100), IDcustom(2), Customcontent(2)
...
Function ImportPrdOptions(IDPrd,tmp_Opt1,tmp_Attr1,tmp_Opt1Req,tmp_Opt1Order)
	If tmp_Opt1<>"" then
			...
			IF tmp_Attr1<>"" then
				...
				IF testErr=1 THEN
					if CheckCount=0 then
						ErrorsReport=ErrorsReport & "<tr><td>" & "Record " & TotalCSVlines & ": Product SKU " & psku & " - attributes list format is incorrect." & "</td></tr>" & vbcrlf
				...
				if ubound(pcv_Arr2)>4 then
					...
					if pcv_Arr2(0)="" then
						...
						if len(rd_Option1)>250 then
							ErrorsReport=ErrorsReport & "<tr><td>" & "Record " & TotalCSVlines & ": The option attribute name '" & rd_Option1 & "' was longer than 250 characters. It has been truncated." & "</td></tr>" & vbcrlf
		...
		if pcv_IDGrp1>"0" then
			...
			if rstemp.eof then
				if CheckCount=0 then
					ErrorsReport=ErrorsReport & "<tr><td>" & "Record " & TotalCSVlines & ": Product SKU " & psku & " - can not add/update product option group because it does not have any imported attributes." & "</td></tr>" & vbcrlf
					PrdWithoutOpts=PrdWithoutOpts+1
					CheckCount=1
				end if
...
<%
	...
	if customfieldsid(0) > -1 then
		customfieldsname(0)=B(customfieldsid(0))
	...
	TempCSV=f.Readline
	TempCSV=replace(TempCSV,"," & chr(34) & chr(34),"DYDNTD")
	TempCSV=replace(TempCSV,chr(34) & chr(34),"&quot;")
	TempCSV=replace(TempCSV,"DYDNTD","," & chr(34) & chr(34))
	TempCSV=replace(TempCSV,"'","''")
	A=split(TempCSV,",")
	RecordError=false
	TotalCSVlines=TotalCSVlines+1

		'Maybe Product Text Fields have commas or "End Text Line - VBCrLf" characters, fix these problems
	 	i=0
	 	j=0
	 	Do while j<fields
	 	if i>ubound(a) then
			ErrorsReport=ErrorsReport & "<tr><td>" & "Record " & TotalCSVlines & ": Does not have enough data fields." & "</td><tr>" & vbcrlf
			RecordError=true
			exit do
		end if
	 	if left(A(i),1)= chr(34) then
	 		IF A(i)=chr(34) & chr(34) then
	 			CSVRecord(j)=""
	 		ELSE
	 		if len(a(i))>1 then
	 		...
	 		if (right(A(i),1)=chr(34)) and (len(a(i))>1) then
	 			...
	 			Do
	 				...
	 				Do
	 					Templine=f.ReadLine
	 					Templine=replace(Templine,"," & chr(34) & chr(34),"DYDNTD")
	 					Templine=replace(Templine,chr(34) & chr(34),"&quot;")
	 					Templine=replace(Templine,"DYDNTD","," & chr(34) & chr(34))
...
if RecordError=false then
		...
		if not rstemp4.eof then
			...
			If err.number<>0 Then
				if (err.number=-2147217904) then
					ErrorsReport=ErrorsReport & "<tr><td>" & "<strong>Record " & TotalCSVlines & " was NOT updated</strong>. There was an error in the import file.  Make sure you are not using text in a number field." & "</td></tr>" & vbcrlf '// display error
				else
					ErrorsReport=ErrorsReport & "<tr><td>" & "<strong>Record " & TotalCSVlines & " was NOT updated</strong>. Check the import file. Error reported:  " & err.description & "</td></tr>" & vbcrlf '// display error
				end if
			...
			'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
			SKUError=1
			ErrorsReport=ErrorsReport & "<tr><td>" & "Record " & TotalCSVlines & ": This SKU is not in the database." & "</td></tr>" & vbcrlf
			RecordError=true
			'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
			'// END: Error in Append
			'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	'********************************************************************************
		...
		'///////////////////////////////////////////////////////////////////////////
			...
			'// START: Sku Exists
			'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
				ErrorsReport=ErrorsReport & "<tr><td>" & "Record " & TotalCSVlines & ": SKU " & psku & " could not be imported because it already exists." & "</td></tr>" & vbcrlf
				RecordError=true
			'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
			'// START: Sku Exists
			'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		...
		'///////////////////////////////////////////////////////////////////////////
			...
			set rstemp=conntemp.execute(query)
			If err.number<>0 Then
				ErrorsReport=ErrorsReport & "<tr><td>" & "<strong>Record " & TotalCSVlines & " NOT imported.</strong> Error Details:  " & err.description & "</td></tr>" & vbcrlf '// display error
				RecordError=true '// do not count import
			End If
			set rstemp=nothing
			'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		...
		ELSE
			...
			if pcategory<>"" then
				...
				if pIdCategory=-1 then
				 ...
				 TempProducts=TempProducts & "Record " & TotalCSVlines & ": Product SKU: " & psku & " - Product Name: " & pname & vbcrlf
				end if
			else
...
IF RecordError=false THEN
	...
	If prd_Opt1<>"" AND InvalidGrp1=0 then
		...
		if Opt1ID<>-1 AND prd_Opt1<>"" AND InvalidGrp1=1 then
			if CheckCount=0 then
				ErrorsReport=ErrorsReport & "<tr><td>" & "Record " & TotalCSVlines & ": Product SKU " & psku & " - can not add/update product option group because the fields are not properly formatted or it does not have any attributes." & "</td></tr>" & vbcrlf
				PrdWithoutOpts=PrdWithoutOpts+1
				CheckCount=1
			end if
		end if
...
<tr>
    ...
    <%end if%>
      ...
      <div>A total of <b><font color="#FF0000"><%=TotalCSVlines-ImportedRecords%></font></b> records <u>could NOT</u> be <%if session("append")="1" then%>updated<%else%>imported<%end if%>. See the Error(s) Report section below for details</div>
    <%end if%>
  </div>
