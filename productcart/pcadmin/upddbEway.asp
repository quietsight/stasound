...
IF request("action")="sql" then
	if request.querystring("hmode")="2" then
		...
		if SSIP="" or UID="" or PWD="" then
			response.redirect "upddbEway.asp?mode=3"
			response.End
		end if
		...
		if instr(ucase(scDSN),"DSN=") then
			response.redirect "upddbEway.asp?mode=1"
			response.End
		end if
		call openDb()
	end if
	...
	'//  Create column eWayCVV for table Eway
	query="ALTER TABLE [Eway] ADD [eWayCVV] [int] NULL"
	...
	if err.number <> 0 then
		if instr(ucase(Err.Description),"COLUMN NAMES IN EACH TABLE MUST BE UNIQUE") then
			...
			ErrStr=ErrStr&"Unable to update TABLE Eway - Error: "&Err.Description&"<BR>"
			err.number=0
			iCnt=iCnt+1
		end if
	else
		query="UPDATE Eway SET eWayCVV=0;"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp.execute(query)
		query="ALTER TABLE [Eway] ADD CONSTRAINT [DF_Eway_eWayCVV] DEFAULT (0) FOR [eWayCVV];"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp.execute(query)
	end if
	
	'//  Create column eWayBeagleActive for table Eway
	query="ALTER TABLE [Eway] ADD [eWayBeagleActive] [int] NULL"
	set rs=server.CreateObject("ADODB.RecordSet")
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		if instr(ucase(Err.Description),"COLUMN NAMES IN EACH TABLE MUST BE UNIQUE") then
		...
		query="UPDATE Eway SET eWayBeagleActive=0;"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp.execute(query)
		query="ALTER TABLE [Eway] ADD CONSTRAINT [DF_Eway_eWayBeagleActive] DEFAULT (0) FOR [eWayBeagleActive];"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp.execute(query)
	end if
...
ELSE
	if request("action")="access" then
		...
		query="ALTER TABLE Eway ADD eWayCVV LONG NULL;"
		set rs=server.CreateObject("ADODB.RecordSet")
		...
		if err.number <> 0 then
			...
			set rs=server.CreateObject("ADODB.RecordSet")
			set rs=connTemp1.execute(query)
			query="ALTER TABLE [Eway] ALTER COLUMN [eWayCVV] LONG NULL DEFAULT '0';"
			set rs=server.CreateObject("ADODB.RecordSet")
			set rs=connTemp1.execute(query)
		end if
		
		query="ALTER TABLE Eway ADD eWayBeagleActive LONG NULL;"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=connTemp1.execute(query)
		if err.number <> 0 then
			call TrapError("Eway")
			...
			set rs=server.CreateObject("ADODB.RecordSet")
			set rs=connTemp1.execute(query)
			query="ALTER TABLE [Eway] ALTER COLUMN [eWayBeagleActive] LONG NULL DEFAULT '0';"
			set rs=server.CreateObject("ADODB.RecordSet")
			set rs=connTemp1.execute(query)
		end if
...
<form action="upddbEway.asp" method="get" name="form1" class="pcForms">
<% 
if mode="complete" then
	response.redirect "AddModRTPayment.asp?gwchoice=31"
	response.end
else %>
	...
	<table class="pcCPcontent" style="width:600px;" align="center">
		...
		<tr>
	      ...
	      <span style="font-weight: bold">You must update your database before you can add/modify eWay Gateway to support Real-Time CVN and Beagle (Geo-IP Anti Fraud).<u></u></span></div>				<p><strong><br>
				    <br>
			...
			<% if scDB="Access" then %>
					<tr>
						<td align="center">
						  <input type="button" name="access" value="Update Your ProductCart MS Access Database" onClick="location='upddbEway.asp?action=access&mode=<%=tmp_mode%>&id=<%=tmp_id%>';" class="submit2">
						</td>
					</tr>
			<% 
				else
			...
			<% end if %>
				<tr>
					...
					<% if request.querystring("mode")="1" OR request.querystring("mode")="3" then %>
						...
						<% else %>
					  <input type="button" name="access2" value="Update Your ProductCart MS SQL Database" onClick="location='upddbEway.asp?action=sql&mode=<%=tmp_mode%>&id=<%=tmp_id%>';" class="submit2">
						<% end if %>
					</td>
			</tr>
			<% end if %>
