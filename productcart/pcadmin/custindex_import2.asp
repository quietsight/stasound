<% pageTitle = "Customer Import Wizard - Locate data file on Web server" %>
<% section = "mngAcc" %>
...
if MTest=1 then
 ...
 Response.redirect "custindex_import.asp?s=1&nextstep=1&msg=ProductCart successfully located the data file " & FName & " on the Web server."
end if
end if
%> 
...
<form method="post" action="custindex_import2.asp?action=check" onSubmit="return Form1_Validator(this)" class="pcForms">
<table class="pcCPcontent">
	<tr>
    	<td colspan="2">Enter the full name of the customer import data file that has already been uploaded to the Web server. <strong>Only *.csv &amp; *.xls</strong> files are accepted. For more information on what fields can be imported and on how to prepare your *.csv &amp; *.xls files for import, please refer to the ProductCart <a href="http://wiki.earlyimpact.com/productcart/customer-import" target="_blank">User Guide</a>.<br>
	<tr>
	...
	<tr>
        <td>File name:
            ...
            &nbsp;<input type="button" name="back" value="Back" onClick="document.location.href='custindex_import.asp'">
		</td>
	</tr>
	</table>
	</form>
