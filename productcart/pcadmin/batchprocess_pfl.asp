		...
		if cdbl(pAmount)>0 then		
			...
			if not IsEmpty(pidAuthOrder) then
				...
				If pFraudMode="1" Then
					nvpstr = "TRXTYPE=U"
					nvpstr = nvpstr &"&TENDER=P"
					nvpstr = nvpstr &"&PARTNER="& pcPay_PayflowLink_Partner
					nvpstr = nvpstr &"&USER="& trim(pcPay_PayflowLink_User)
					nvpstr = nvpstr &"&VENDOR="& trim(pcPay_PayflowLink_Vendor)
					nvpstr = nvpstr &"&PWD="& pcPay_PayflowLink_Password
				...
				If scPPLogging = "1" Then
					...
					OutputFile.WriteLine "Request from ProductCart to PayPal for Payflow Link Capture"
					OutputFile.WriteBlankLines(1)
					OutputFile.WriteLine "TEST: " &pcPay_PayflowLink_Sandbox
					OutputFile.WriteBlankLines(1)
				...
				'//PAYPAL LOGGING START
				If scPPLogging = "1" Then
					OutputFile.WriteLine "Response from PayPal to ProductCart for Payflow Link Capture"
