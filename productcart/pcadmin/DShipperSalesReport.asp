...
<html>
<head>
	<title>Online Sales Report - Sales by Drop-Shipper</title>
...
pIDDropShipper=request("IDDropShipper")
if pIDDropShipper="" then
	response.redirect "menu.asp"
end if

query="SELECT pcDropShipper_Company,pcDropShipper_FirstName,pcDropShipper_LastName FROM pcDropShippers WHERE pcDropShipper_ID=" & pIDDropShipper
set rs1=conntemp.execute(query)
DropShipperName=rs1("pcDropShipper_Company") & " (" & rs1("pcDropShipper_FirstName") & " " & rs1("pcDropShipper_LastName") & ")"
set rs1=nothing

query="SELECT DISTINCT orders.idorder,orders.idcustomer,orders.total,orders.shipmentDetails,orders.paymentDetails,orders.taxAmount,orders.discountDetails,orders.orderDate,orders.processDate,orders.shipDate FROM (ProductsOrdered INNER JOIN Orders ON ProductsOrdered.idOrder=Orders.idOrder) INNER JOIN Products ON ProductsOrdered.idProduct=Products.idProduct WHERE ProductsOrdered.pcDropShipper_ID=" & pIDDropShipper & " AND Products.pcSupplier_ID<>" & pIDDropShipper & " AND ((orders.orderStatus>2 AND orders.orderStatus<5) OR (orders.orderStatus>6 AND orders.orderStatus<9) OR (orders.orderStatus=10 OR orders.orderStatus=12)) " & TempSQL1 & TempSQL2 & TempSpecial & " ORDER BY " & tmpD & " DESC;"
...
<% Else %>
	<tr> 
		<td colspan="10"><h2>Total Sales recorded from: <%=strTDateVar%> to: <%=strTDateVar2%> <%if DropShipperName<>"" then%>&nbsp;for <%=DropShipperName%><%end if%></h2></td>
	</tr>
	<tr> 
...
do until rs.EOF
	...
	query="SELECT Products.idproduct,ProductsOrdered.quantity,(ProductsOrdered.unitPrice*ProductsOrdered.quantity) AS SubPTotal,Products.Description  FROM (ProductsOrdered INNER JOIN Orders ON ProductsOrdered.idOrder=Orders.idOrder) INNER JOIN Products ON ProductsOrdered.idProduct=Products.idProduct WHERE ProductsOrdered.pcDropShipper_ID=" & pIDDropShipper & " AND Products.pcSupplier_ID<>" & pIDDropShipper & "  and Orders.idOrder=" &  pc_idorder & " AND ((orders.orderStatus>2 AND orders.orderStatus<5) OR (orders.orderStatus>6 AND orders.orderStatus<9) OR (orders.orderStatus=10 OR orders.orderStatus=12)) " & TempSQL1 & TempSQL2 & TempSpecial & " ORDER BY Products.Description ASC;"
