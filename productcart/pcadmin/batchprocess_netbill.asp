...
query="SELECT NBAccountID,NBSiteTag FROM netbill Where idNetbill=1"
set rs=server.CreateObject("ADODB.RecordSet")
set rs=conntemp.execute(query)
dim NBAccountID,NBSiteTag
...
for r=1 to checkboxCnt
	...
	if request.Form("checkOrd"&r)="YES" then
		...
		netbillamount=request.Form("netbillamount"&r)
		curamount=request.Form("curamount"&r)
		pidnetbillorder=request.Form("idnetbillorder"&r)
		pOrderStatus=request.Form("orderstatus"&r)
		pCheckEmail=request.Form("checkEmail"&r)
		pTempIdOrder=request.Form("idOrder"&r)
		
		pIdOrder=(int(pTempIdOrder)-scpre)
		qry_ID=pIdOrder
		idOrder=pIdOrder

		'send pre-auth-capture
		strRequest ="account_id=" & NBAccountId&_
		"&tran_type=D"&_
		"&pay_type=C"&_
		"&site_tag="&NBSiteTag&_
		"&orig_id=" & Request.Form("trans_id"&r)  &_
		"&bill_name_1=" & Request.Form("fName"&r)  &_
		"&bill_name_2=" & Request.Form("lName"&r)  &_
		"&bill_street=" & Request.Form("address"&r)  &_
		"&bill_zip=" & Request.Form("zip"&r) &_
		"&bill_country=" & Request.Form("countryCode"&r)  &_
		"&card_number=" & Request.Form("ccnum"&r)  &_
		"&card_expire=" & Request.Form("ccexp"&r) &_
		"&amount=" & curamount&_
		"&cust_ip="&sIPAddress
		...
		If objHTTP.Status = 200 Then  ' HTTP_STATUS_OK=200 
			...
			strAuthMsg=objDictResponse.Item("auth_msg")
			strTicketCode=objDictResponse.Item("ticket_code")
			strAVSCode=objDictResponse.Item("avs_code")
			strAuthDate=objDictResponse.Item("auth_date")
			strStatusCode=objDictResponse.Item("status_code")
			strTransID=objDictResponse.Item("trans_id")
			strAuthCode=objDictResponse.Item("auth_code")
			strCVV2Code=objDictResponse.Item("cvv2_code")
		else
			strStatusCode=00
		end if
		
		'Check NetBilling response code 1=approved 2=declined 3=error.
		if strStatusCode = 1 then
			'order was successful
			call opendb()
			
			'update authorders to captured
			query="UPDATE netbillorders SET captured=1 WHERE idnetbillorder="&pidnetbillorder&";"
			set rstemp=connTemp.execute(query)
			set rstemp=nothing
			
			'purge cc data
			query="SELECT ccnum, pcSecurityKeyID FROM netbillorders WHERE idnetbillorder="&pidnetbillorder&";"
			set rstemp=connTemp.execute(query)
			...
			query="UPDATE netbillorders SET ccnum='"&tempfour&"' WHERE idnetbillorder="&pidnetbillorder&";"
			set rstemp=connTemp.execute(query)
			set rstemp=nothing
			...
			failedData=failedData&"Order Number: "& (int(pIdOrder)+scpre) &" was NOT updated, " & objHttp.statusText & "<BR>" 
