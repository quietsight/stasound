...
<%
pageTitle="Purge All Deleted Products and Related Orders from Database"
...
If request("submit")<>"" then
	...
	' get products
	query="SELECT products.idProduct FROM products WHERE products.removed <> 0;"
	set rs = Server.CreateObject("ADODB.Recordset")
	set rs = conntemp.execute(query)
	if rs.eof then
		set rs=nothing
		call closeDB()
		response.redirect "PurgeRemovedProducts.asp?msg=2"
	end if
	...
	FOR i=0 TO pcProudctsCount
			...
			if err.number <> 0 then
				...
				set rs=nothing
				call closeDb()
				response.redirect "techErr.asp?error="& Server.Urlencode("Error in Purging All Deleted Products: " & pcErrorNumber & " - " & pcErrorDescription) 
			end If
			...
			do until rs.eof
				...
				set rs2=conntemp.execute(query)
					if err.number <> 0 then
						...
						set rs=nothing
						call closeDb()
						response.redirect "techErr.asp?error="& Server.Urlencode("Error 10a purging product on PurgeRemovedProducts.asp. Details: " & errDesc) 
					end If
				...
				set rs2=conntemp.execute(query)
					if err.number <> 0 then
						...
						set rs=nothing
						call closeDb()
						response.redirect "techErr.asp?error="& Server.Urlencode("Error 10b purging product on PurgeRemovedProducts.asp. Details: " & errDesc) 
					end If
				...
				set rs2=conntemp.execute(query)
					if err.number <> 0 then
						...
						set rs=nothing
						call closeDb()
						response.redirect "techErr.asp?error="& Server.Urlencode("Error 10c purging product on PurgeRemovedProducts.asp. Details: " & errDesc) 
					end If
				...
				set rs2=conntemp.execute(query)
					if err.number <> 0 then
						...
						set rs=nothing
						call closeDb()
						response.redirect "techErr.asp?error="& Server.Urlencode("Error 10d purging product on PurgeRemovedProducts.asp. Details: " & errDesc) 
					end If
	...
	set rs=nothing
	call closeDb()
	response.redirect "PurgeRemovedProducts.asp?msg=1&n="&pcProudctsDeletedCount
...
ELSE 
			...
			if validNum(msg) then
				...
				if msg="2" then
					response.write "<div class=pcCPmessage>There are no deleted products to remove. <a href=purgeProducts.asp>Other options</a>.</div>"
				else
					response.write "<div class=pcCPmessageSuccess>"& pcProudctsDeletedCount & " products were permanently removed from the database. <a href=menu.asp>Continue</a>.</div>"
				end if
			else
...
%>
			...
			<form action="PurgeRemovedProducts.asp" method="post" name="form" id="form" class="pcForms">
				<table class="pcCPcontent">
					<tr> 
						<td>
                          <div class="pcCPmessage" style="color: #F00; font-weight: bold; font-size: 18px;">WARNING: This is a dangerous feature.</div>
                          <p>This feature allows you to completely remove all previously deleted products from your database. When you take this action you will <u>completely <strong>remove all previously deleted products</strong></u> and <u><strong>all orders containing those products</strong></u>. This action is permanent and cannot be undone.</p>
                          <p style="margin-top: 8px;">There are <strong><%=productCount%></strong> deleted products in the database. Here are the <% if productCount>4 then%> first 5 <%end if%>products.</p>
                          
							<% '// Start Removed Products Preview
					...
					<tr> 
						<td align="center">
						<input type="submit" value="Remove All Previously Deleted Products" name="submit" class="submit2" OnClick="return confirm('You are about to permanently remove all previously deleted products from your database. Are you sure that you wish to continue? This action cannot be undone. Click OK to continue or Cancel to abort.');" />&nbsp;
