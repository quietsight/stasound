...
if ((right(ucase(filepathname),4)=".CSV") or (right(ucase(filepathname),4)=".XLS")) AND IsUploadAllowed(filepathname) then
	...
	Sub BuildUploadRequest(RequestBin)
		...
		Response.redirect "custindex_import.asp?s=1&nextstep=1&msg=Customer data file: " & ucase(lgCSV) & " was uploaded successfully."
	end if
else
...
If InValidCSV>0 then
	...
	if pcTmpErrSize = 1 then
		...
		response.redirect "custindex_import1.asp?s=0&msg="&Server.URLEncode("Invalid file type. Only CSV & XLS files can be uploaded to the server.")
	end if
	'//END DETAILED MSG
Else
	response.redirect "custindex_import1.asp?s=0&msg=You did not select a file to upload."
end if
