...
Dim rs, connTemp, query, idProduct, pcIntConfigOnly, pcIntserviceSpec
	...
	' This page is used to link to the product edit page
	...
	if Cint(pcIntConfigOnly) <> 0 Then ' This is a Build To Order Only item
			response.redirect "modifyProduct.asp?idProduct="&idProduct&"&prdType=item" & tabQS
		elseif Cint(pcIntserviceSpec) <> 0 Then ' This is a Built To Order product
			response.redirect "modifyProduct.asp?idProduct="&idProduct&"&prdType=bto" & tabQS
		else ' This is a standard product
			response.redirect "modifyProduct.asp?idProduct="&idProduct&"&prdType=std" & tabQS
	end if
%>