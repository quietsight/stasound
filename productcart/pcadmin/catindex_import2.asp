<% pageTitle = "Category Import Wizard - Locate data file on Web server" %>
<% section = "products" %>
...
if MTest=1 then
 ...
 Response.redirect "catindex_import.asp?s=1&nextstep=1&msg=ProductCart successfully located the data file " & FName & " on the Web server."
end if
end if
%> 
...
<tr>
	...
	<form method="post" action="catindex_import2.asp?action=check" onSubmit="return Form1_Validator(this)" class="pcForms">
<table class="pcCPcontent">
	<tr>
    	<td colspan="2">Enter the full name of the category data file that has already been uploaded to the Web server. <strong>Only *.csv &amp; *.xls</strong> files are accepted. For more information on what fields can be imported and on how to prepare your *.csv &amp; *.xls files for import, please refer to the ProductCart <a href="http://wiki.earlyimpact.com/productcart/products_category_import" target="_blank">User Guide</a>.
        </td>
    </tr>
	<tr>
    ...
    <tr>
        <td>File name:
            ...
            &nbsp;<input type="button" name="back" value="Back" onClick="document.location.href='catindex_import.asp'">
        </td>
    </tr>
</table>
</form>
