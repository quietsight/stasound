...
<% '--Updater File-- %>
<% pageTitle = "eMoney Gateway - Database Update" %>
<% Section="paymntOpt" %>
...
IF request("action")="sql" then
	if request.querystring("hmode")="2" then
		...
		if SSIP="" or UID="" or PWD="" then
			response.redirect "upddbEMoney.asp?mode=3"
			response.End
		end if
		...
		if instr(ucase(scDSN),"DSN=") then
			response.redirect "upddbEMoney.asp?mode=1"
			response.End
		end if
		call openDb()
	end if
	...
	'**** Create table pcPay_eMoney ***************************************
	query="CREATE TABLE [dbo].[pcPay_eMoney] ("
	query=query&"[pcPay_EM_ID] [int] NULL  DEFAULT (1),"
	query=query&"[pcPay_EM_MerchantID] [nvarchar] (50) NULL ,"
	query=query&"[pcPay_EM_MerchantPassword] [nvarchar] (250) NULL ,"
	query=query&"[pcPay_EM_cardTypes] [nvarchar] (50) NULL ,"
	query=query&"[pcPay_EM_CVC] [int] NULL ,"
	query=query&"[pcPay_EM_TestMode] [int] NULL DEFAULT (0) "
	query=query&");"
	set rs=server.CreateObject("ADODB.RecordSet")
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		TrapSQLError("pcPay_eMoney")
	else
		query="INSERT INTO pcPay_eMoney (pcPay_EM_ID, pcPay_EM_CVC, pcPay_EM_TestMode) VALUES (1,0,1);"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp.execute(query)
	end if
...
ELSE
	if request("action")="access" then
		...
		'**** Create table pcPay_eMoney ***************************************
		query="CREATE TABLE pcPay_eMoney ("
		query=query&"pcPay_EM_ID INTEGER NOT NULL DEFAULT 1 ,"
		query=query&"pcPay_EM_MerchantPassword VarChar(50) NULL ,"
		query=query&"pcPay_EM_MerchantID VarChar(250) NULL ,"
		query=query&"pcPay_EM_cardTypes VarChar(50) NULL ,"
		query=query&"pcPay_EM_CVC INTEGER NOT NULL DEFAULT 0 ,"
		query=query&"pcPay_EM_TestMode INTEGER NOT NULL DEFAULT 0 "
		query=query&");"
		
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp1.execute(query)
		if err.number <> 0 then
			TrapError("pcPay_eMoney")
		else
			query="INSERT INTO pcPay_eMoney (pcPay_EM_ID, pcPay_EM_CVC, pcPay_EM_TestMode) VALUES (1,0,1);"
			set rs=server.CreateObject("ADODB.RecordSet")
			set rs=conntemp1.execute(query)
...
<form action="upddbEMoney.asp" method="get" name="form1" class="pcForms">
<%
if mode="complete" then 
	response.redirect "AddModRTPayment.asp?gwchoice=54"
	response.end
else %>
	<table class="pcCPcontent" style="width:600px;" align="center">
		...
		<tr>
			<td>
				...
				<span style="font-weight: bold">You must update your database before you can add eMoney Gateway.<u></u></span><br>
				In order to activate eMoney real-time payment gateway you will need to update your ProductCart database to add the required table.</div>				<p><strong><br>
				    <br>
			...
			<% if scDB="Access" then %>
					<tr>
						<td align="center">
						  <input type="button" name="access" value="Update Your ProductCart MS Access Database" onClick="location='upddbEMoney.asp?action=access';" class="submit2">
						</td>
					</tr>
			<% 
				else
			...
			<% end if %>
				<tr>
					...
					<% if request.querystring("mode")="1" OR request.querystring("mode")="3" then %>
						...
						<% else %>
					  <input type="button" name="access2" value="Update Your ProductCart MS SQL Database" onClick="location='upddbEMoney.asp?action=sql';" class="submit2">
						<% end if %>
					</td>
			</tr>
			<% end if %>
