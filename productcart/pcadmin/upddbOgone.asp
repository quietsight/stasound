...
IF request("action")="sql" then
	if request.querystring("hmode")="2" then
		...
		if SSIP="" or UID="" or PWD="" then
			response.redirect "upddbOgone.asp?mode=3"
			response.End
		end if
		...
		if instr(ucase(scDSN),"DSN=") then
			response.redirect "upddbOgone.asp?mode=1"
			response.End
		end if
		call openDb()
	end if
	...
	query="CREATE TABLE [dbo].[pcPay_Ogone] ("
	query=query&"[pcPay_OG_ID] [int] NULL  DEFAULT (1),"
	query=query&"[pcPay_OG_MerchantID] [nvarchar] (100) NULL ,"
	query=query&"[pcPay_OG_MerchantPassword] [nvarchar] (250) NULL ,"
	query=query&"[pcPay_OG_TransType] [nvarchar] (3) NULL ,"
	query=query&"[pcPay_OG_Lang] [nvarchar] (5) NULL ,"
	query=query&"[pcPay_OG_CurCode] [nvarchar] (3) NULL ,"
	query=query&"[pcPay_OG_cardTypes] [nvarchar] (250) NULL ,"
	query=query&"[pcPay_OG_CVC] [int] NULL ,"
	query=query&"[pcPay_OG_AccountID] [nvarchar] (100) NULL ,"
	query=query&"[pcPay_OG_TestMode] [int] NULL DEFAULT (0) "
	query=query&");"
	set rs=server.CreateObject("ADODB.RecordSet")
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		TrapSQLError("pcPay_Ogone")
	else
		query="INSERT INTO pcPay_Ogone (pcPay_OG_ID, pcPay_OG_CVC, pcPay_OG_TestMode) VALUES (1,0,1);"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp.execute(query)
	end if
...
ELSE
	if request("action")="access" then
		...
		'**** Create table pcPay_Ogone ***************************************
		query="CREATE TABLE pcPay_Ogone ("
		query=query&"pcPay_OG_ID INTEGER NOT NULL DEFAULT 1 ,"
		query=query&"pcPay_OG_MerchantID VarChar(100) NULL ,"
		query=query&"pcPay_OG_MerchantPassword VarChar(250) NULL ,"
		query=query&"pcPay_OG_TransType VarChar(3) NULL ,"
		query=query&"pcPay_OG_Lang VarChar(5) NULL ,"
		query=query&"pcPay_OG_CurCode VarChar(3) NULL ,"
		query=query&"pcPay_OG_cardTypes VarChar(250) NULL ,"
		query=query&"pcPay_OG_CVC INTEGER NOT NULL DEFAULT 0 ,"
		query=query&"pcPay_OG_AccountID VarChar(100) NULL ,"
		query=query&"pcPay_OG_TestMode INTEGER NOT NULL DEFAULT 0 "
		query=query&");"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp1.execute(query)
		if err.number <> 0 then
			TrapError("pcPay_Ogone")
		else
			query="INSERT INTO pcPay_Ogone (pcPay_OG_ID, pcPay_OG_CVC, pcPay_OG_TestMode) VALUES (1,0,1);"
			set rs=server.CreateObject("ADODB.RecordSet")
			set rs=conntemp1.execute(query)
...
<form action="upddbOgone.asp" method="get" name="form1" class="pcForms">
<%
if mode="complete" then 
	response.redirect "AddModRTPayment.asp?gwchoice=55"
	response.end
else %>
	<table class="pcCPcontent" style="width:600px;" align="center">
		...
		<tr>
			...
			<% if scDB="Access" then %>
					<tr>
						<td align="center">
						  <input type="button" name="access" value="Update Your ProductCart MS Access Database" onClick="location='upddbOgone.asp?action=access';" class="submit2">
						</td>
					</tr>
			<% 
				else
			...
			<% end if %>
				<tr>
					...
					<% if request.querystring("mode")="1" OR request.querystring("mode")="3" then %>
						...
						<% else %>
					  <input type="button" name="access2" value="Update Your ProductCart MS SQL Database" onClick="location='upddbOgone.asp?action=sql';" class="submit2">
						<% end if %>
					</td>
			</tr>
			<% end if %>
