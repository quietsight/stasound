...
ELSE
	if request("action")="access" then
		...
		'**** Create table pcPay_TGS ***************************************
		query="CREATE TABLE pcPay_TGS ("
		query=query&"pcPay_TGS_ID INTEGER NOT NULL DEFAULT 1 ,"
		query=query&"pcPay_TGS_AccountID VarChar(20) NULL ,"
		query=query&"pcPay_TGS_CUR VarChar(3) NULL ,"
		query=query&"pcPay_TGS_AuthKey VarChar(255) NULL ,"
		query=query&"pcPay_TGS_TransType VarChar(20) NULL ,"
		query=query&"pcPay_TGS_TestMode VarChar(5) NULL ,"
		query=query&"pcPay_TGS_CVV2 INTEGER NULL DEFAULT '0'"
		query=query&");"
		set rs=server.CreateObject("ADODB.RecordSet")
...
if mode="complete" then 
	response.redirect "AddModRTPayment.asp?gwchoice=65"
	response.end
else %>
	<table class="pcCPcontent" style="width:600px;" align="center">
		...
		<tr>
			...
			<% if scDB="Access" then %>
					<tr>
						<td align="center">
						  <input type="button" name="access" value="Update Your ProductCart MS Access Database" onClick="location='upddbTGS.asp?action=access';" class="submit2">
						</td>
					</tr>
			<% 
				else
			...
			<% end if %>
				<tr>
					...
					<% if request.querystring("mode")="1" OR request.querystring("mode")="3" then %>
						...
						<% else %>
					  <input type="button" name="access2" value="Update Your ProductCart MS SQL Database" onClick="location='upddbTGS.asp?action=sql';" class="submit2">
						<% end if %>
					</td>
			</tr>
			<% end if %>
