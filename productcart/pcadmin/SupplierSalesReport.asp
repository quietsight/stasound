...
<html>
<head>
	<title>Online Sales Report - Sales by Supplier</title>
...
query="SELECT pcSupplier_Company,pcSupplier_FirstName,pcSupplier_LastName FROM pcSuppliers WHERE pcSupplier_ID=" & pIDSupplier
set rs1=conntemp.execute(query)
SupplierName=rs1("pcSupplier_Company") & " (" & rs1("pcSupplier_FirstName") & " " & rs1("pcSupplier_LastName") & ")"
set rs1=nothing

query="SELECT DISTINCT orders.idorder,orders.idcustomer,orders.total,orders.shipmentDetails,orders.paymentDetails,orders.taxAmount,orders.discountDetails,orders.orderDate,orders.processDate,orders.shipDate FROM (ProductsOrdered INNER JOIN Orders ON ProductsOrdered.idOrder=Orders.idOrder) INNER JOIN Products ON ProductsOrdered.idProduct=Products.idProduct WHERE Products.pcSupplier_ID=" & pIDSupplier & " AND ((orders.orderStatus>2 AND orders.orderStatus<5) OR (orders.orderStatus>6 AND orders.orderStatus<9) OR (orders.orderStatus=10 OR orders.orderStatus=12)) " & TempSQL1 & TempSQL2 & TempSpecial & " ORDER BY " & tmpD & " DESC;"
...
<% Else %>
	<tr> 
		<td colspan="10"><h2>Total Sales recorded from: <%=strTDateVar%> to: <%=strTDateVar2%> <%if SupplierName<>"" then%>&nbsp;for <%=SupplierName%><%end if%></h2></td>
	</tr>
	<tr> 
...
do until rs.EOF
	...
	query="SELECT Products.idproduct,ProductsOrdered.quantity,(ProductsOrdered.unitPrice*ProductsOrdered.quantity) AS SubPTotal,Products.Description  FROM (ProductsOrdered INNER JOIN Orders ON ProductsOrdered.idOrder=Orders.idOrder) INNER JOIN Products ON ProductsOrdered.idProduct=Products.idProduct WHERE Products.pcSupplier_ID=" & pIDSupplier & " and Orders.idOrder=" &  pc_idorder & " AND ((orders.orderStatus>2 AND orders.orderStatus<5) OR (orders.orderStatus>6 AND orders.orderStatus<9) OR (orders.orderStatus=10 OR orders.orderStatus=12)) " & TempSQL1 & TempSQL2 & TempSpecial & " ORDER BY Products.Description ASC;"
