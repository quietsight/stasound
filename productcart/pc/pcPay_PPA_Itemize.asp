...
'// Order number is valid
		...
		do while not rsOrdObj.eof
			...
			QDiscTotal = QDiscTotal + QDiscounts
			...
			'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
			if instr(pdescription, "&") then
				LNameStr = "&L_NAME"&count&"["&len(pdescription)&"]"
			else
				LNameStr = "&L_NAME"&count
			end if
			iString = iString & LNameStr&"="& pdescription
			iString = iString & "&L_COST"&count&"="& punitPrice1
			iString = iString & "&L_QTY"&count&"="& pquantity
			
			ItemTotal = Ccur(ItemTotal) + (punitPrice1*pquantity)
			'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		...
		'**************************************************************
		If QDiscTotal>0 Then
			QDiscTotal = "-" & QDiscTotal
			iQDString = iQDString & "&L_NAME"&count&"=Product Quantity Discounts"
			iQDString = iQDString & "&L_COST"&count&"="& QDiscTotal
			iQDString = iQDString & "&L_QTY"&count&"=1"
			ItemTotal = Ccur(ItemTotal) + Ccur(QDiscTotal)
			count=count+1
		End If
		...
		if PayCharge>0 then
			...
			iPCString = iPCString & "&L_NAME"&count&"="& dictLanguage.Item(Session("language")&"_PayPal_7")
			iPCString = iPCString & "&L_COST"&count&"="& tmpFinalPayCharge
			iPCString = iPCString & "&L_QTY"&count&"=1"
			
			ItemTotal = Ccur(ItemTotal) + Ccur(tmpFinalPayCharge)
			count=count+1
		Else
		...
		if pCatDiscounts>0 then
			...
			if NOT pcv_strPayPalmethod="PayPalStandard" then
				...
				iCDString = iCDString & "&L_NAME"&count&"="& dictLanguage.Item(Session("language")&"_PayPal_8")
				iCDString = iCDString & "&L_COST"&count&"="& tmpFinalCatDiscounts
				iCDString = iCDString & "&L_QTY"&count&"=1"
				
				ItemTotal = Ccur(ItemTotal) + Ccur(tmpFinalCatDiscounts)
			end if
			count=count+1
		Else
		...
		if pGWTotal>0 then		
			...
			if NOT pcv_strPayPalmethod="PayPalStandard" then
				...
				iGWString = iGWString & "&L_NAME"&count&"="& dictLanguage.Item(Session("language")&"_PayPal_9")
				iGWString = iGWString & "&L_COST"&count&"="& tmpFinalGiftWrapping
				iGWString = iGWString & "&L_QTY"&count&"=1"
				
				ItemTotal = Ccur(ItemTotal) + Ccur(tmpFinalGiftWrapping)
			end if
			count=count+1
		Else
		...
		ItemTotal = pcv_strFinalTotal - (pcv_strFinalShipCharge+pcv_strFinalServiceCharge+pcv_strFinalTax)
