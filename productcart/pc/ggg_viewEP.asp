...
function update()
{
	...
	doc.write('<IMG SRC="' + imagename + '"><\/td><\/tr><tr><td><%if MACBrowser=0 then%><form name="viewn"><input type="image" id="submit" src="images/close.gif" align="right" value="Close Window" onClick="self.close()"><%end if%><\/td><\/tr><\/table>');
...
<tr>
	...
	<td valign="top">
		<%if pImageUrl<>"" then%>
			<img src='catalog/<%=pImageUrl%>' alt="" hspace="10"> 
				...
				<tr> 
					   <td valign="bottom" align="right">
							...
							<% Else If InStr(Request.ServerVariables("HTTP_USER_AGENT"),"Safari") Then 
								Response.Write("<a href=""catalog//"&pLgimageURL&""" target=""_blank"">")
							Else
								Response.Write("<a href=""catalog//"&pLgimageURL&""" target=""_blank"">")
							End If
							End If %>
						   	<img src="<%=rsIconObj("zoom")%>" border="0" hspace="10"></a>
						</td>
				</tr>
				</table>
			<% end if
		'if no image, show no_image.gif
			...
			<img src='catalog/no_image.gif' alt="Product image not available" width="100" height="100">
		<% end if%>
	</td>
...
<% if trim(psDesc) <> "" then %>
<tr>
	<td colspan="2"><hr size="1" noshade color="<%=AColor%>"></td>
</tr>
<tr>
	<td colspan="2"><a name="details"></a>&nbsp;</td>
</tr>
<tr>
	<td colspan="2">
...
<tr>
	<td colspan="2">
		<a href="javascript:history.go(-1);"><img src="<%=rslayout("RetRegistry")%>" alt="<%response.write dictLanguage.Item(Session("language")&"_viewEP_6")%>" border=0></a>
	</td>
</tr>
</table>
</div>
