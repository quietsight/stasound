<%
Function updPrices(pcv_chkIDProduct,pcv_chkIDConfig)
...
'End Recalculate Default Price
	...
	for i=lbound(ArrProduct) to (UBound(ArrProduct)-1)
		...
		do while not rsA.eof
				...
				TempD1=0
				if (clng(ArrQuantity(i)*pQty1)>=clng(QFrom)) and (clng(ArrQuantity(i)*pQty1)<=clng(QTo)) then
				if QPercent="-1" then
				if session("customerType")=1 then
				TempD1=ArrQuantity(i)*pQty1*ArrPrice(i)*0.01*DWUnit
				else
				TempD1=ArrQuantity(i)*pQty1*ArrPrice(i)*0.01*DUnit
				end if
				else
				if session("customerType")=1 then
				TempD1=ArrQuantity(i)*pQty1*DWUnit
				else
				TempD1=ArrQuantity(i)*pQty1*DUnit
				end if
				end if
				end if
