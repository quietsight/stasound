...
if request("PaymentSubmitted")="Go" then
	...
	If (strStatus <> 200) Then
		...
		elseif session("x_response_code")<>1 then
			response.redirect "msgb.asp?message="&server.URLEncode("<b>Error&nbsp;"&session("x_response_code")&"</b>: "& session("x_response_reason_text")&"<br><br><a href="""&tempURL&"?psslurl=gwAuthorizeCheck.asp&idOrder="&session("GWOrderId")&"""><img src="""&rslayout("back")&""" border=0></a>")
			response.end
		End If
	end if
			...
			<td>
				<form method="POST" action="<%=session("redirectPage")%>" name="PaymentForm" class="pcForms">
					...
					<tr> 
						...
						<td><%=scCurSign&money(pcBillingTotal)%></td>
					</tr>
                    <tr><td colspan="2" class="pcSpacer"></td></tr>
                    <tr><td colspan="2"><p>By clicking the &quot;Place Order&quot; button below, I authorize <%= scCompanyName %> to charge my <%=x_bank_acct_type%> account on <%=Now()%> for the amount of <%=scCurSign&money(pcBillingTotal)%>.</p></td></tr>
                    <tr><td colspan="2" class="pcSpacer"></td></tr>
