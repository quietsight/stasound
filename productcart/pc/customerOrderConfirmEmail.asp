...
If Trim(pshippingAddress) <> "" Then
	...
	customerEmail=customerEmail & trim(pshippingPhone) & vbCrLf
Else
...
Do While Not rsOrderDetails.EOF
	...
	If len(xfdetails)>3 then
		...
		for q=lbound(xfarray) to ubound(xfarray)
			customerEmail=customerEmail & xfarray(q)
	...
	if pcPrdOrd_BundledDisc>0 then
		customerEmail=customerEmail & FixedField(50, "L", dictLanguage.Item(Session("language")&"_custOrdInvoice_36"))
		customerEmail=customerEmail & FixedField(10, "R", " ")
		customerEmail=customerEmail & FixedField(10, "R","-" & scCurSign & money(pcPrdOrd_BundledDisc))  & vbCrLf
	end if
	customerEmail=customerEmail & vbCrLf
	...
	'======================================
		if (QDiscounts<>"") and (QDiscounts<>"0") then
			customerEmail=customerEmail & FixedField(50, "L", dictLanguage.Item(Session("language")&"_adminMail_32"))
			customerEmail=customerEmail & FixedField(10, "R", " ")
	...
	if cdblCmprTmp2<>cdblCmprTmp1 then
		customerEmail=customerEmail & FixedField(50, "L", dictLanguage.Item(Session("language")&"_adminMail_33"))
		customerEmail=customerEmail & FixedField(10, "R", " ")
	...
	IF DPOrder="1" AND pOrderStatus="3" then
		...
		do while not rs11.eof
			...
			if not rs.eof then
				...
				CustomerEmail=customerEmail & dictLanguage.Item(Session("language")&"_sendMail_28") & pProductName & vbcrlf & vbcrlf
	...
	IF pGCs="1" AND pOrderStatus="3" then
		...
		do while not rs11.eof
			...
			if not rs.eof then
				...
				CustomerEmail=customerEmail & dictLanguage.Item(Session("language")&"_sendMail_43") & pName & vbcrlf
...
'********************************************************
	...
	IF pGCs="1" AND pOrderStatus="3" THEN
		...
		if GcReMsg<>"" then
		ReciEmail3=replace(dictLanguage.Item(Session("language")&"_sendMail_72"),"<customer name>",pCustomerFullName) & vbcrlf & GcReMsg & vbcrlf
		else
		ReciEmail3=""
		end if
