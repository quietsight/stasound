...
session("redirectPage")="gwMoneris2.asp"
...
query="SELECT pcPay_Moneris_StoreId, pcPay_Moneris_Key, pcPay_Moneris_TransType, pcPay_Moneris_Lang, pcPay_Moneris_Testmode, pcPay_Moneris_CVVEnabled, pcPay_Moneris_Meth, pcPay_Moneris_Interac FROM pcPay_Moneris Where pcPay_Moneris_Id=1;"
...
'*************************************************************************************
	...
	if strMessage<>"" then
		response.redirect "msgb.asp?message="&server.URLEncode("<b>Errors&nbsp;</b>:<br>"&strMessage&"<br><br><a href="""&tempURL&"?psslurl=gwMoneris2.asp&idCustomer="&session("idCustomer")&"&idOrder="&session("GWOrderId")&"""><img src="""&rslayout("back")&"""></a>")
	end if

	'// Validates expiration
	if DateDiff("d", Month(Now)&"/"&Year(now), request("expMonth")&"/20"&request("expYear"))<=-1 then
		response.redirect "msgb.asp?message="&server.URLEncode("<b>Error</b>: "& dictLanguage.Item(Session("language")&"_paymntb_o_6")&"<br><br><a href="""&tempURL&"?psslurl=gwMoneris2.asp&idCustomer="&session("idCustomer")&"&idOrder="&session("GWOrderId")&"""><img src="""&rslayout("back")&"""></a>")
	end if

	if NOT CheckCC(request.Form("CardNumber")) then
		response.redirect "msgb.asp?message="&server.URLEncode("<b>Error</b>: "& dictLanguage.Item(Session("language")&"_paymntb_o_5")&"<br><br><a href="""&tempURL&"?psslurl=gwMoneris2.asp&idCustomer="&session("idCustomer")&"&idOrder="&session("GWOrderId")&"""><img src="""&rslayout("back")&"""></a>")
	end if
	...
	stext=stext & "&order_id="&pcStrCustRefID
