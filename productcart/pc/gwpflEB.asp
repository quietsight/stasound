...
If scPPLogging = "1" Then
	...
	'// Create Log of request string and save in PFLLog.txt
...
session("redirectPage")="gwpflEB.asp"
...
pcPay_PayflowLink_User=rs("v_User")
pcPay_PayflowLink_User=enDeCrypt(pcPay_PayflowLink_User, scCrypPass)					
pcPay_PayflowLink_Partner=rs("v_Partner")				
pcPay_PayflowLink_Password=rs("v_Password")
pcPay_PayflowLink_Password=enDeCrypt(pcPay_PayflowLink_Password, scCrypPass)					
pcPay_PayflowLink_MerchantLogin=rs("v_Vendor")	
pcPay_PayflowLink_MerchantLogin=enDeCrypt(pcPay_PayflowLink_MerchantLogin, scCrypPass)					
pcPay_PayflowLink_Vendor=pcPay_PayflowLink_MerchantLogin
pcPay_PayflowLink_TransType=rs("pfl_transtype")			
pcPay_PayflowLink_CSC=rs("pfl_CSC")
pcPay_PayflowLink_Sandbox=rs("pfl_testmode")				

if ucase(pcPay_PayflowLink_Sandbox) = "YES" then
...
stext = "USER="&trim(pcPay_PayflowLink_User)
stext = stext & "&VENDOR="&trim(pcPay_PayflowLink_Vendor)
stext = stext & "&PARTNER="&pcPay_PayflowLink_Partner
stext = stext & "&PWD="&pcPay_PayflowLink_Password
stext = stext & "&TRXTYPE="&pcPay_PayflowLink_TransType
...
If scPPLogging = "1" Then
	...
	OutputFile.WriteLine "Request from ProductCart to PayPal for Payflow Link"
...
'//PAYPAL LOGGING START
If scPPLogging = "1" Then
	OutputFile.WriteLine "Response from PayPal to ProductCart for Payflow Link"
...
dim ppmode
ppmode = ""
if ucase(pcPay_PayflowLink_Sandbox) = "YES" then ppmode = "MODE=TEST&"

'//PAYPAL LOGGING START
If scPPLogging = "1" Then
	OutputFile.WriteLine "iFrame generated for Payflow Link"
