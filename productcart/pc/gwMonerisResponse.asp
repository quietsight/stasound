...
if session("IDEBIT_ISSCONF") <> "" then 
	strDeclinedRedirect = "msgb.asp?message="&server.URLEncode("<b>Errors&nbsp;</b>:&nbsp;"&session("Message")&"<br><br><a href="""&tempURL&"?psslurl=gwMonerisInterac.asp&idCustomer="&session("idCustomer")&"&idOrder="&session("GWOrderId")&"""><img src="""&rslayout("back")&"""></a>")
else
	strDeclinedRedirect = "msgb.asp?message="&server.URLEncode("<b>Errors&nbsp;</b>:&nbsp;"&session("Message")&"<br><br><a href="""&tempURL&"?psslurl=gwMoneris2.asp&idCustomer="&session("idCustomer")&"&idOrder="&session("GWOrderId")&"""><img src="""&rslayout("back")&"""></a>")
end if

strStatus=getUserInput(request("status"),0)
select case ucase(strStatus)
	case "VALID-APPROVED"
		...
		call opendb()
		query="INSERT INTO pcPay_OrdersMoneris (pcPay_MOrder_OrderID,pcPay_MOrder_TransKey, pcPay_MOrder_Result,pcPay_MOrder_responseId,pcPay_MOrder_responseCode,pcPay_MOrder_DateStamp,pcPay_MOrder_TimeStamp,pcPay_MOrder_Bankcode,pcPay_MOrder_Transname,pcPay_MOrder_cardholder,pcPay_MOrder_total,pcPay_MOrder_card,pcPay_MOrder_f4l4,pcPay_MOrder_expDate,pcPay_MOrder_message,pcPay_MOrder_ISOcode,pcPay_MOrder_TransId,pcPay_MOrder_IDEBIT_ISSCONF,pcPay_MOrder_IDEBIT_ISSNAME ) VALUES ("&tempOrdId&",'"&session("moneris_transaction Key")&"','"&session("moneris_result")&"','"&session("moneris_response_order_id")&"','"&session("moneris_response_code")&"','"&session("moneris_date_stamp")&"','"&session("moneris_time_stamp")&"','"&session("moneris_bank_approval_code")&"','"&session("moneris_trans_name")&"','"&session("moneris_cardholder")&"','"&session("moneris_charge_total")&"','"&session("moneris_card")&"','"&session("moneris_f4l4")&"','"&session("moneris_expiry_date")&"','"&session("moneris_message")&"','"&session("moneris_iso_code")&"','"&session("moneris_bank_transaction_id")&"','"&left(replace(session("IDEBIT_ISSCONF"),"'","''"),15)&"','"&left(replace(session("IDEBIT_ISSNAME"),"'","''"),30)&"');"
		...
		session("IDEBIT_ISSCONF") = ""
		session("IDEBIT_ISSNAME") = ""			
