...
<div id="pcShowCart">
    ...
    <br />
		...
		<% 
        Dim strSSCLink
        strSSCLink="onepagecheckout.asp"
        if scSSL="1" AND scIntSSLPage="1" then
            strSSCLink=replace((scSslURL&"/"&scPcFolder&"/pc/onepagecheckout.asp"),"//","/")
            strSSCLink=replace(strSSCLink,"https:/","https://")
            strSSCLink=replace(strSSCLink,"http:/","http://")
        end if
        %>
		<%
		...
		if pcv_strShowCheckoutBtn=1 then
			if Session("Cust_GW")="1" then
				pcv_Link="cRec.asp?actGW=Hello"
			else
				if vcHaveGcsTest=1 then
					pcv_Link="cRec.asp?actGCs=Hello"
				else
					pcv_Link=strSSCLink
				end if
			end if
			
		%> - <a href=<%=pcv_Link%>><strong><%=dictLanguage.Item(Session("language")&"_smallcart_5")%></strong></a>
		<% end if %>
