...
If ack=0 Then
	...
	if session("GWTransId") <> "" then
		...
		If PaymentAction="A" Then
			...
			query="INSERT INTO pcPay_PayPal_Authorize (idOrder, amount, paymentmethod, transtype, authcode, idCustomer, captured, AuthorizedDate, CurrencyCode, gwCode) VALUES ("&pcGatewayDataIdOrder&", "&pcBillingTotal&", 'PayPalExp', 'Authorization', '"&session("GWTransId")&"', "&pcIdCustomer&", 0," & tmpStr & ", '"&pcPay_PayPal_Currency&"', 9);"
			set rs=server.CreateObject("ADODB.RecordSet")
			set rs=connTemp.execute(query)				
			if err.number<>0 then
				call LogErrorToDatabase()
