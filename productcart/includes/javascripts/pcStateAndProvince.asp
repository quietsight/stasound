...
// START: Switch Zones
function SwitchStates(a,countrybox,statebox,provincebox,SelectedSession,FormInstance) {
var targeBox = eval("document.<%=pcv_strTargetForm%>." + countrybox + "").options;
	...
	for (var i=0; i<totalNumStates; i++) {     
		if (States[i][1] == 1) { // if the zone is "1"
			if (States[i][0] == b) { // if we have a relationship, populate the states					
				...
				OverRideReqField('false','<%=pcv_isStateCodeRequired%>', FormInstance);
				LabelField(States[i][0],countrybox);
				FilterStates(States[i][0],statebox,SelectedSession);
				// clear the province box
				var pBox = eval("document.<%=pcv_strTargetForm%>." + provincebox + "");
				pBox.value="";
				break;
			}
		} else {
			...
			OverRideReqField('<%=pcv_isProvinceCodeRequired%>','false', FormInstance);
		}
	}		
}
// END: Switch Zones
...
// START: Filter States
function FilterStates(array_name,statebox, SelectedSession) {	
	var targeBox = eval("document.<%=pcv_strTargetForm%>." + statebox + "").options;												
	targeBox.length = 0;
...
function SelectState(countrybox,statebox,provincebox,SelectedSession,FormInstance) {
	...
	if (targeBoxIndex > 0) {
		// Display the Zone	
		SwitchStates(targeBoxIndex,countrybox,statebox,provincebox,SelectedSession,FormInstance);
	}
}									
// END: Check Selected State

// START: Fill hidden text param
function OverRideReqField(a, b, FormInstance) {
	var prov = eval("document.<%=pcv_strTargetForm%>.pcv_isProvinceCodeRequired"+FormInstance+"");
	var state = eval("document.<%=pcv_strTargetForm%>.pcv_isStateCodeRequired"+FormInstance+"");
	prov.value = a;
	state.value = b;
}									
// END: Check Selected State
...
%>
		...
		<tr id="zone1<%=pcv_strCountryBox%>" style="display: none;"> 
			<td><p ID="Label<%=pcv_strCountryBox%>"><%=pcv_strSPFieldLabel%></p></td>
			<td>
				<p>									
				<% 
				...
				%>
				<select name="<%=pcv_strTargetBox%>" size="1">
					<option value=""></option>
					<% 
					do while not rs.eof
		...
		<tr id="zone2<%=pcv_strCountryBox%>" style="display:;"> 
			<td><p ID="Label2<%=pcv_strCountryBox%>"><%=pcv_strSPFieldLabel%></p></td>
			<td>										
			<p>
...
<script type="text/javascript" language="JavaScript">
	SelectState('<%=pcv_strCountryBox%>', '<%=pcv_strTargetBox%>', '<%=pcv_strProvinceBox%>', '<%=pcv_strTargetBoxValue%>', '<%=pcv_strFormInstance%>');
</script>		
...
Public Sub pcs_CountryDropdown
	...
	%>
	<tr> 
		<td><p><%response.write dictLanguage.Item(Session("language")&"_CustAddModShip_8")%></p></td>
		<td>
		<p>
		<%
		...
		%>
				...
				<select name="<%=pcv_strCountryBox%>" size="1" onchange="SwitchStates(this.options.selectedIndex, '<%=pcv_strCountryBox%>', '<%=pcv_strTargetBox%>', '<%=pcv_strProvinceBox%>', '<%=pcv_strTargetBoxValue%>', '<%=pcv_strFormInstance%>');">
					<option></option>
					<% 
					For i = pcv_intLBound To pcv_intUBound
						'// get nested array
